#!/usr/bin/env python3
"""
Parallel implementation of WBC1 block cipher algorithm using MPI (mpi4py) with CACHED OPERATIONS.

PERFORMANCE OPTIMIZED VERSION:
This version pre-computes and caches all operation permutations during cipher initialization,
...
class WBC1Cipher:
    """WBC1 block cipher implementation with configurable parameters and cached operations."""
    
    def __init__(self, key: bytes, block_size: int = 16, num_rounds: int = 16, algorithm_mode: int = 1):
        """
        Initialize WBC1 cipher with cached operation permutations.
        ...
        # PERFORMANCE OPTIMIZATION: Pre-compute and cache all operation permutations
        # This eliminates SHA256 computation and permutation generation on every block
        self._operation_cache = self._precompute_operation_permutations()
    ...
    def _precompute_operation_permutations(self) -> Dict:
        """
        Pre-compute all operation permutations at initialization time.
        This caches the SHA256 computations and permutation arrays,
        eliminating the need to recompute them for each block.
        
        Returns:
            Dictionary mapping (operation, inverse) to (forward_perm, inverse_perm) tuples
        """
        cache = {}
        
        # Pre-compute permutations for all base operations
        for op_id, op in self.operations.items():
            op_type = op[0]
            
            if op_type == 'dynamic':
                # For dynamic operations, cache the sub-operation permutations
                for subop in op[2]:
                    subop_str = str(subop)
                    if subop_str not in cache:
                        forward_perm, inverse_perm = self._compute_operation_permutation(subop)
                        cache[subop_str] = (forward_perm, inverse_perm)
            else:
                # For simple operations, cache directly
                op_str = str(op)
                if op_str not in cache:
                    forward_perm, inverse_perm = self._compute_operation_permutation(op)
                    cache[op_str] = (forward_perm, inverse_perm)
        
        return cache
    
    def _compute_operation_permutation(self, op) -> Tuple[np.ndarray, np.ndarray]:
        """
        Compute forward and inverse permutation arrays for an operation.
        This is called once per unique operation during initialization.
        """
        ...
        forward_perm = np.arange(self.block_size, dtype=np.int32)
        rng.shuffle(forward_perm)
        
        # Compute inverse permutation
        inverse_perm = np.zeros(self.block_size, dtype=np.int32)
        for i in range(self.block_size):
            inverse_perm[forward_perm[i]] = i
    ...
    def _apply_operation(self, block: np.ndarray, op_id: int, inverse=False) -> np.ndarray:
        """
        Apply a dynamic operation to the block using cached permutations.
        This version uses pre-computed permutations for performance.
        """
        ...
        if op_type == 'dynamic':
            for subop in (reversed(op[2]) if inverse else op[2]):
                block = self._apply_single_operation_cached(block, subop, inverse)
            return block
        
        return self._apply_single_operation_cached(block, op, inverse)
    
    def _apply_single_operation_cached(self, block: np.ndarray, op, inverse=False) -> np.ndarray:
        """
        Apply a single operation using cached permutation.
        This eliminates SHA256 computation and permutation generation per block.
        """
        op_str = str(op)
        forward_perm, inverse_perm = self._operation_cache[op_str]
        
        if inverse:
            return block[inverse_perm]
        else:
            return block[forward_perm]
