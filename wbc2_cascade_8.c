/*
 * WBC2-CASCADE-8 -- Eksperyment: 8 раундів × 4 composed-перестановки
 * =====================================================================
 * Базується на wbc2_cascade_new.c, ключові відмінності:
 *
 *   ROUNDS = 8  (замість 32)
 ...
 *   2. XOR:    buf[i] ^= key_mat[(i + r*PERMS_PER_ROUND) % KEY_SIZE]
 *   3. S-box:  buf[i] = sbox[buf[i]]
 *   4. rotate: bitwise_rotate_cube(key_mat[r*4] & 7, right)
 *
 ...
 *   Кешується по (key_mat + block_size); перебудовується при зміні ключа
 *
 * Криптостійкість vs wbc2_cascade_new (32 rounds):
 *   - S-box застосовується 8×  (замість 32×) → менша нелінійність per pass
 *   - Але: DOUBLE pass = ефективно 16 S-box застосувань
 *   - Composed perm ≡ тому ж самому ефекту що 32 окремих перестановки
 *   - Рекомендується використовувати з cascade=DOUBLE
 *
 * Швидкодія (fixed key, без cascade):
 *   - Побудова composed tables: 8 rounds × 4 ops × N = 32N (разово)
...
static int auto_block_size(int data_len) {
    for (int i=0;i<NUM_CUBE_SIZES;i++) if (data_len<=CUBE_SIZES[i].n) return CUBE_SIZES[i].n;
    return CUBE_SIZES[NUM_CUBE_SIZES-1].n;
}
static int dim_for(int block_size) {
    for (int i=0;i<NUM_CUBE_SIZES;i++) if (CUBE_SIZES[i].n==block_size) return CUBE_SIZES[i].dim;
    return 0;
}
...
static void sha256(const uint8_t *d, size_t l, uint8_t o[32]) { SHA256(d,l,o); }
static void sha512(const uint8_t *d, size_t l, uint8_t o[64]) { SHA512(d,l,o); }
...
static void rot90_2d(uint8_t *m, int d, int k) {
    k=((k%4)+4)%4; uint8_t t[d*d];
    for(int t2=0;t2<k;t2++){
        for(int i=0;i<d;i++) for(int j=0;j<d;j++) t[i*d+j]=m[j*d+(d-1-i)];
        memcpy(m,t,d*d);
    }
}
...
static void rotate_slice(Cube *c,int ax,int pos,int k){ uint8_t sl[c->dim*c->dim]; get_slice(c,ax,pos,sl); rot90_2d(sl,c->dim,k); set_slice(c,ax,pos,sl); }
static void rotate_whole_cube(Cube *c,int ax,int k) {
    k=((k%4)+4)%4; int d=c->dim,total=d*d*d; uint8_t tmp[total];
    for(int t=0;t<k;t++){
        int a0=ax,a1=(ax+1)%3;
        for(int i=0;i<d;i++) for(int j=0;j<d;j++) for(int k2=0;k2<d;k2++){
            int co[3]={i,j,k2};
            int na0=co[a1],na1=d-1-co[a0];
            int nc[3]={i,j,k2}; nc[a0]=na0; nc[a1]=na1;
            tmp[idx3(d,nc[0],nc[1],nc[2])]=c->data[idx3(d,i,j,k2)];
        }
        memcpy(c->data,tmp,total);
    }
}
static void diagflip(Cube *c,int ax) {
    int d=c->dim,total=d*d*d; uint8_t tmp[total];
    for(int i=0;i<d;i++) for(int j=0;j<d;j++) for(int k=0;k<d;k++){
        int di,dj,dk;
        if(ax==0){di=i;dj=k;dk=j;} else if(ax==1){di=k;dj=j;dk=i;} else{di=j;dj=i;dk=k;}
        tmp[idx3(d,di,dj,dk)]=c->data[idx3(d,i,j,k)];
    }
    memcpy(c->data,tmp,total);
}
static void swap_layers(Cube *c,int ax,int i1,int i2) {
    int d=c->dim; i1=((i1%d)+d)%d; i2=((i2%d)+d)%d;
    uint8_t s1[d*d],s2[d*d]; get_slice(c,ax,i1,s1); get_slice(c,ax,i2,s2);
    set_slice(c,ax,i1,s2); set_slice(c,ax,i2,s1);
}
static void bitwise_rotate_cube(Cube *c,int n,int right) {
    int t=c->dim*c->dim*c->dim;
    for(int i=0;i<t;i++) c->data[i]=right?rotate_right(c->data[i],n):rotate_left(c->data[i],n);
}
...
static void mt_seed(MT *m,uint32_t seed){ m->mt[0]=seed; for(int i=1;i<MT_N;i++) m->mt[i]=1812433253UL*(m->mt[i-1]^(m->mt[i-1]>>30))+(uint32_t)i; m->idx=MT_N; }
static uint32_t mt_rand(MT *m){
    if(m->idx>=MT_N){ for(int i=0;i<MT_N;i++){ uint32_t y=(m->mt[i]&0x80000000UL)|(m->mt[(i+1)%MT_N]&0x7fffffffUL); m->mt[i]=m->mt[(i+397)%MT_N]^(y>>1); if(y&1)m->mt[i]^=2567483615UL; } m->idx=0; }
    uint32_t y=m->mt[m->idx++]; y^=y>>11; y^=(y<<7)&2636928640UL; y^=(y<<15)&4022730752UL; y^=y>>18; return y;
}
...
static void mt_shuffle(MT *m,int *arr,int n){ for(int i=n-1;i>0;i--){ int j=mt_randint(m,0,i); int tmp=arr[i]; arr[i]=arr[j]; arr[j]=tmp; } }
static uint32_t sha256_seed(const uint8_t *h32){ return ((uint32_t)h32[28]<<24)|((uint32_t)h32[29]<<16)|((uint32_t)h32[30]<<8)|(uint32_t)h32[31]; }
...
static int dir_char_to_k(char d,int inv){ int k; switch(d){case '\'':k=-1;break;case '2':k=2;break;case '3':k=3;break;default:k=1;break;} return inv?-k:k; }
...
static void apply_alg_string(Cube *cube,const char *s,int inv){
    ...
    char *t=strtok(buf," "); while(t&&nt<255){toks[nt++]=t;t=strtok(NULL," ");}
...
static void build_op_table(const uint8_t key[32]){
    for(int i=0;i<NUM_OPS;i++){
        uint8_t h[32],buf[KEY_SIZE+11]; memcpy(buf,key,KEY_SIZE); memcpy(buf+KEY_SIZE,"WBC1_OP",7);
        ...
        for(int j=0;j<cl;j++) build_prim_op_from_base_idx(&g_ops[i].moves[j],mt_choice(&mt,N_STATIC_BASE));
    }
...
/* ── permutation table (base, build once per block_size) ───────────────── */
 ...
 *   Applying perm p after existing comp C:
 *   new_comp[i] = C[p[i]]
 *
 * Cache key: (key_mat[32] + block_size)
 * In cascade mode: rebuilt every block (1 rebuild + 1 apply = ~2.7× less work
 *                   than 32 separate apply_operations).
 * In fixed-key mode: built once, amortized to negligible.
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static void build_comp_table(const uint8_t *key_mat, int N) {
    ...
    for (int r = 0; r < ROUNDS; r++) {
         ...
         * to apply perm p AFTER existing comp: new[i] = comp[p[i]] */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * KEY-DEPENDENT S-BOX  (same as wbc2_cascade_new)
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static void build_sbox(const uint8_t *key) {
    ...
    for(int i=255;i>0;i--){ uint32_t j=mt_rand(&mt)%(uint32_t)(i+1); uint8_t t=g_sbox[i]; g_sbox[i]=g_sbox[j]; g_sbox[j]=t; }
    for(int i=0;i<256;i++) g_inv_sbox[g_sbox[i]]=(uint8_t)i;
    memcpy(g_sbox_key,key,KEY_SIZE); g_sbox_valid=1;
}

/* ── padding ────────────────────────────────────────────────────────────── */

static uint8_t *wbc1_pad(const uint8_t *data, size_t data_len, int block_bytes, size_t *out_len) {
    int rem=(int)(data_len%block_bytes), fill=(rem==0)?0:block_bytes-rem;
    size_t aligned=data_len+fill, total=aligned+block_bytes;
    uint8_t *out=(uint8_t*)calloc(total,1); memcpy(out,data,data_len);
    out[aligned]=(uint8_t)(fill&0xFF); out[aligned+1]=(uint8_t)((fill>>8)&0xFF);
    *out_len=total; return out;
}
static uint8_t *wbc1_unpad(const uint8_t *data, size_t data_len, int block_bytes, size_t *out_len) {
    ...
    size_t strip=block_bytes+fill;
    if(strip>data_len){*out_len=0;return (uint8_t*)calloc(1,1);}
    *out_len=data_len-strip;
    uint8_t *out=(uint8_t*)malloc(*out_len+1); memcpy(out,data,*out_len); out[*out_len]=0; return out;
}

/* ── derive_cascade_key ─────────────────────────────────────────────────── */

static void derive_cascade_key(const uint8_t *enc_block, int block_bytes, uint8_t new_rk[KEY_SIZE]) {
    uint8_t S[64]={0};
...
/* ═══════════════════════════════════════════════════════════════════════════
 * BLOCK CIPHER  (WBC2-8: 8 rounds × composed_perm → XOR → S-box → rotate)
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static void encrypt_block(const uint8_t *key_mat, const uint8_t *block,
    ...
    /* 8 rounds: composed_perm → XOR → S-box → rotate */
    for (int r=0; r<ROUNDS; r++) {
        const uint16_t *comp = g_comp_fwd + (size_t)r * N;
        uint8_t tmp[N];

        /* a) Composed permutation (4 ops in one pass) */
        for (int i=0;i<N;i++) tmp[i] = buf[comp[i]];
        memcpy(buf, tmp, N);

        /* b) XOR with round key material */
        int key_off = (r * PERMS_PER_ROUND) % KEY_SIZE;
        for (int i=0;i<N;i++) buf[i] ^= key_mat[(key_off + i) % KEY_SIZE];

        /* c) Key-dependent S-box */
        for (int i=0;i<N;i++) buf[i] = g_sbox[buf[i]];

        /* d) Cyclic bit rotation (using first op_id of the round) */
...
static const char *mode_name(EncMode m){
    switch(m){case MODE_ECB:return "ECB";case MODE_CBC:return "CBC";
              ...
              case MODE_CTR:return "CTR";case MODE_WBC_CTR_HMAC:return "WBC-CTR-HMAC";default:return "?";}
}
static int mode_is_key_avalanche(EncMode m){ return m==MODE_CTR||m==MODE_CFB||m==MODE_OFB||m==MODE_WBC_CTR_HMAC; }
...
static uint8_t *cascade_encrypt(const uint8_t master_key[KEY_SIZE],
    ...
    build_op_table(master_key); g_perm_bs=-1; build_perm_table(block_bytes); g_comp_valid=0;
    ...
    uint8_t rk_bwd[KEY_SIZE]; derive_cascade_key(master_key,KEY_SIZE,rk_bwd); g_comp_valid=0;
...
static uint8_t *cascade_decrypt(const uint8_t master_key[KEY_SIZE],
    ...
    uint8_t rk[KEY_SIZE]; memcpy(rk,master_key,KEY_SIZE); g_comp_valid=0;
    uint8_t prev_iv[4096],enc_prev[4096],tmp[4096];
    if(iv) memcpy(prev_iv,iv,block_bytes); else memset(prev_iv,0,block_bytes);
    ...
    for(size_t b=0;b<n_blocks;b++){
        ...
        if(mode==MODE_ECB){
            ...
            for(int i=0;i<block_bytes;i++) out_b[i]=tmp[i]^prev_iv[i];
            derive_cascade_key(blk,block_bytes,rk); memcpy(prev_iv,blk,block_bytes);
        } else if(mode==MODE_CFB){
            encrypt_block(rk,prev_iv,block_size,enc_prev);
            derive_cascade_key(enc_prev,block_bytes,rk);
            for(int i=0;i<block_bytes;i++) out_b[i]=blk[i]^enc_prev[i];
            memcpy(prev_iv,blk,block_bytes);
        } else if(mode==MODE_OFB){
            encrypt_block(rk,prev_iv,block_size,enc_prev);
            derive_cascade_key(enc_prev,block_bytes,rk);
            for(int i=0;i<block_bytes;i++) out_b[i]=blk[i]^enc_prev[i];
            memcpy(prev_iv,enc_prev,block_bytes);
        } else if(mode==MODE_CTR){
            encrypt_block(rk,prev_iv,block_size,enc_prev);
            derive_cascade_key(enc_prev,block_bytes,rk);
            for(int i=0;i<block_bytes;i++) out_b[i]=blk[i]^enc_prev[i];
            int carry=1; for(int i=block_bytes-1;i>=0&&carry;i--){int v=prev_iv[i]+carry;prev_iv[i]=(uint8_t)(v&0xFF);carry=v>>8;}
        }
    }
...
static uint8_t *cascade_encrypt_ctr_hmac(const uint8_t key[KEY_SIZE],
                                           ...
                                           size_t *out_len) {
    int block_bytes=block_size;
    build_op_table(key); g_perm_bs=-1; build_perm_table(block_bytes); g_comp_valid=0;
    ...
    int ctr_tail=(block_bytes>NONCE_SIZE)?block_bytes-NONCE_SIZE:1;
    for(size_t b=0;b<n_blocks;b++){
        uint8_t ctr[4096];
        for(int j=0;j<block_bytes;j++) ctr[j]=rk[j%KEY_SIZE];
        for(int j=0;j<NONCE_SIZE&&j<block_bytes;j++) ctr[j]^=nonce[j];
        for(int i=ctr_tail-1;i>=0;i--) ctr[NONCE_SIZE+i]^=(uint8_t)(b>>(8*(ctr_tail-1-i)));
        uint8_t enc_ctr[4096];
        encrypt_block(rk,ctr,block_size,enc_ctr);
...
static uint8_t *cascade_decrypt_ctr_hmac(const uint8_t key[KEY_SIZE],
    ...
    build_op_table(key); g_perm_bs=-1; build_perm_table(block_size); g_comp_valid=0;
    ...
    uint8_t *res=wbc1_unpad(plain,cipher_len,block_bytes,out_len);
...
static double shannon_entropy(const uint8_t *d, size_t l) {
    if(!l)return 0.0; size_t f[256]={0}; for(size_t i=0;i<l;i++) f[d[i]]++;
    double h=0.0; for(int i=0;i<256;i++) if(f[i]){double p=(double)f[i]/l;h-=p*log2(p);} return h;
}
static double chi_square(const uint8_t *d, size_t l) {
    if(!l)return 0.0; size_t f[256]={0}; for(size_t i=0;i<l;i++) f[d[i]]++;
    double e=(double)l/256.0,c=0.0; for(int i=0;i<256;i++){double x=(double)f[i]-e;c+=x*x/e;} return c;
}
static double correlation(const uint8_t *x, const uint8_t *y, size_t n) {
    if(!n)return 0.0; double mx=0,my=0; for(size_t i=0;i<n;i++){mx+=x[i];my+=y[i];}mx/=n;my/=n;
    double cov=0,vx=0,vy=0; for(size_t i=0;i<n;i++){double a=x[i]-mx,b=y[i]-my;cov+=a*b;vx+=a*a;vy+=b*b;}
...
static void statistics_tests(const uint8_t *plain, size_t plain_len,
    ...
    size_t cmp=plain_len<cipher_len?plain_len:cipher_len;
    printf("  Correlation plain<->cipher: %.4f             (ideal ~0)\n",correlation(plain,cipher,cmp));
    int reps=0; for(size_t i=1;i<cipher_len;i++) if(cipher[i]==cipher[i-1]) reps++;
...
static void run_self_tests(const uint8_t key[32], EncMode mode, int double_pass) {
    printf("\n%s\n","────────────────────────────────────────────────────────────");
    printf("  Self-tests  |  mode=%s  cascade=%s  [WBC2-8 rounds=%d perms/round=%d]\n",
    ...
    for(int t=0;t<n;t++){
        ...
        if(mode==MODE_WBC_CTR_HMAC){
            ...
            free(enc);if(dec)free(dec);if(iv)free(iv);
        }
        passed+=ok;
    ...
    for(size_t i=0;i<flip_count;i++){
        ...
        if(is_stream){
            ...
            if(mode==MODE_WBC_CTR_HMAC){
                ...
                size_t hdr=MAGIC_LEN+1+2+NONCE_SIZE; const uint8_t *cmp=enc1+hdr; size_t cl=enc_len1-hdr-MAC_SIZE;
                if(cl==ref_len) for(size_t j=0;j<cl;j++) total_flips+=__builtin_popcount(ref[j]^cmp[j]);
            } else {
...
static int cmp_double_asc(const void *a,const void *b){ double x=*(double*)a,y=*(double*)b; return x<y?-1:x>y?1:0; }
...
static EncMode parse_mode_arg(const char *v, EncMode def_mode) {
    if (!v) return def_mode;
    if (strcmp(v, "1") == 0 || strcmp(v, "ecb") == 0) return MODE_ECB;
    if (strcmp(v, "2") == 0 || strcmp(v, "cbc") == 0) return MODE_CBC;
    if (strcmp(v, "3") == 0 || strcmp(v, "cfb") == 0) return MODE_CFB;
    if (strcmp(v, "4") == 0 || strcmp(v, "ofb") == 0) return MODE_OFB;
    if (strcmp(v, "5") == 0 || strcmp(v, "ctr") == 0) return MODE_CTR;
    if (strcmp(v, "6") == 0 || strcmp(v, "wbc-ctr-hmac") == 0 || strcmp(v, "hmac") == 0)
        return MODE_WBC_CTR_HMAC;
    return def_mode;
}

static int run_encrypt_task_cli(const uint8_t key[KEY_SIZE], EncMode mode, int double_pass,
                                int data_kb, const char *text) {
    const uint8_t *tdata = NULL;
    size_t tlen = 0;
    uint8_t *rand_buf = NULL;

    if (text && *text) {
        tdata = (const uint8_t *)text;
        tlen = strlen(text);
    } else if (data_kb > 0) {
        tlen = (size_t)data_kb * 1024;
        rand_buf = (uint8_t *)malloc(tlen);
        if (!rand_buf) return 1;
        RAND_bytes(rand_buf, (int)tlen);
        tdata = rand_buf;
    } else {
        const char *demo = "WBC2-CASCADE-8 CLI demo text";
        tdata = (const uint8_t *)demo;
        tlen = strlen(demo);
    }

    int bsz = auto_block_size((int)tlen);
    size_t enc_len = 0, dec_len = 0;
    uint8_t *iv = NULL, *enc = NULL, *dec = NULL;
    size_t ivl = 0;
    ...
    if (mode == MODE_WBC_CTR_HMAC) {
        ...
        enc = cascade_encrypt(key, tdata, tlen, mode, NULL, bsz, &iv, &ivl, double_pass, &enc_len);
        dec = cascade_decrypt(key, enc, enc_len, mode, iv, bsz, double_pass, &dec_len);
    }

    int ok = dec && dec_len == tlen && memcmp(dec, tdata, dec_len) == 0;
    printf("\n  Encrypt task | mode=%s cascade=%s\n", mode_name(mode), double_pass ? "DOUBLE" : "SINGLE");
    printf("  Input: %zu bytes  |  block=%d\n", tlen, bsz);
    printf("  Encrypted: %zu bytes\n", enc_len);
    printf("  Integrity: %s\n\n", ok ? "OK" : "FAIL");

    if (rand_buf) free(rand_buf);
    if (iv) free(iv);
    free(enc);
    if (dec) free(dec);
    return ok ? 0 : 1;
...
static void run_analysis_task_cli(const uint8_t key[KEY_SIZE], EncMode mode, int double_pass, int data_kb) {
    if (data_kb < 1) data_kb = 1;
    size_t tlen = (size_t)data_kb * 1024;
    uint8_t *tdata = (uint8_t *)malloc(tlen);
    if (!tdata) {
        printf("OOM\n");
        return;
    }
    RAND_bytes(tdata, (int)tlen);

    int bsz = auto_block_size((int)tlen);
    size_t enc_len = 0;
    uint8_t *iv = NULL, *enc = NULL;
    size_t ivl = 0;
    ...
    printf("\n  Analysis task | mode=%s cascade=%s size=%d KB\n",
           mode_name(mode), double_pass ? "DOUBLE" : "SINGLE", data_kb);
...
static void benchmark(const uint8_t key[32], EncMode mode, int double_pass) {
    ...
    for(int si=0;si<ns;si++){
        ...
        int block_size=auto_block_size(sz); int all_ok=1;

        {size_t el=0,dl=0;uint8_t *iv=NULL;size_t ivl=0;uint8_t *e=NULL,*d=NULL;
         ...
         else{e=cascade_encrypt(key,data,sz,mode,NULL,block_size,&iv,&ivl,double_pass,&el);d=cascade_decrypt(key,e,el,mode,iv,block_size,double_pass,&dl);if(iv)free(iv);}
         free(e);if(d)free(d);}

        double enc_times[64]={0},dec_times[64]={0};
        for(int r=0;r<repeats;r++){
            struct timespec t0,t1;
            size_t enc_len=0,dec_len=0;uint8_t *enc=NULL,*dec=NULL,*iv=NULL;size_t ivl=0;
            clock_gettime(CLOCK_MONOTONIC,&t0);
            if(mode==MODE_WBC_CTR_HMAC) enc=cascade_encrypt_ctr_hmac(key,data,sz,block_size,NULL,&enc_len);
            else enc=cascade_encrypt(key,data,sz,mode,NULL,block_size,&iv,&ivl,double_pass,&enc_len);
        ...
        double enc_avg=aggregate_bench_time(enc_times,repeats,trim);
        double dec_avg=aggregate_bench_time(dec_times,repeats,trim);
...
static void benchmark_fixed_key(const uint8_t key[KEY_SIZE]) {
    ...
    build_op_table(key); build_perm_table(N); g_comp_valid=0; build_sbox(key);

    /* Pre-build composed table once */
    ...
    printf("  WBC2-8 round: composed_perm(%d ops) → XOR → S-box → rotate  [%d rounds]\n",PERMS_PER_ROUND,ROUNDS);
    printf("  Composed table: 8 × %d = %d entries (built once)\n",N,ROUNDS*N);
    printf("  Encrypt:  %6.1f MB/s",spd_e);
    if(cpu>0) printf("  CPB: %.2f",cpu*1e6/(spd_e*1e6));
    printf("\n  Decrypt:  %6.1f MB/s",spd_d);
...
static EncMode select_mode(void) {
    printf("  Select mode:\n    1. ECB\n    2. CBC\n    3. CFB\n    4. OFB\n    5. CTR\n    6. WBC-CTR-HMAC\n  Mode [1-6, default=1]: ");
    fflush(stdout);
    char buf[8]={0}; if(!fgets(buf,sizeof(buf),stdin)) return MODE_ECB;
    switch(atoi(buf)){case 2:return MODE_CBC;case 3:return MODE_CFB;case 4:return MODE_OFB;case 5:return MODE_CTR;case 6:return MODE_WBC_CTR_HMAC;default:return MODE_ECB;}
}
...
int main(int argc, char *argv[]) {
    int double_pass=1;
    int use_named_cli=0;
    int task=-1; /* 0=encrypt, 1=analysis, 2=selftest, 3=benchmark, 4=fixed */
    int data_kb=1;
    const char *text_arg=NULL;
    EncMode mode=MODE_ECB;

    for(int i=1;i<argc;i++){
        if(strncmp(argv[i],"--",2)==0){ use_named_cli=1; break; }
    }

    if(use_named_cli){
        for(int i=1;i<argc;i++){
            if(strcmp(argv[i],"-s")==0||strcmp(argv[i],"--single")==0||strcmp(argv[i],"--once")==0){
                double_pass=0;
                continue;
            }
            ...
            if(strcmp(argv[i],"--task-encrypt")==0||strcmp(argv[i],"--encrypt")==0) task=0;
            else if(strcmp(argv[i],"--task-analysis")==0||strcmp(argv[i],"--analysis")==0||strcmp(argv[i],"--stats")==0||strcmp(argv[i],"--task-stats")==0) task=1;
            else if(strcmp(argv[i],"--task-selftest")==0||strcmp(argv[i],"--selftest")==0||strcmp(argv[i],"--test")==0) task=2;
            else if(strcmp(argv[i],"--task-benchmark")==0||strcmp(argv[i],"--task-behchmark")==0||strcmp(argv[i],"--benchmark")==0||strcmp(argv[i],"--bench")==0) task=3;
            else if(strcmp(argv[i],"--task-fixed")==0||strcmp(argv[i],"--fixed")==0) task=4;
            else if((strcmp(argv[i],"--task")==0||strcmp(argv[i],"-t")==0) && i+1<argc){
                const char *v=argv[++i];
                if(strcmp(v,"0")==0||strcmp(v,"encrypt")==0||strcmp(v,"enc")==0) task=0;
                else if(strcmp(v,"1")==0||strcmp(v,"analysis")==0||strcmp(v,"stats")==0) task=1;
                ...
                else if(strcmp(v,"4")==0||strcmp(v,"fixed")==0) task=4;
            } else if((strcmp(argv[i],"--mode")==0||strcmp(argv[i],"-m")==0) && i+1<argc){
                mode=parse_mode_arg(argv[++i],mode);
            } else if((strcmp(argv[i],"--size")==0||strcmp(argv[i],"--data-kb")==0) && i+1<argc){
                data_kb=atoi(argv[++i]);
                if(data_kb<1)data_kb=1;
            } else if(strcmp(argv[i],"--text")==0 && i+1<argc){
                text_arg=argv[++i];
            } else if(strcmp(argv[i],"-h")==0||strcmp(argv[i],"--help")==0){
                printf("Usage (named): %s --task <encrypt|analysis|selftest|benchmark|fixed> [--mode ecb|cbc|cfb|ofb|ctr|wbc-ctr-hmac] [--size KB] [--text STRING] [--single|--double]\n",argv[0]);
                printf("Aliases: --task-encrypt, --task-analysis, --task-selftest, --task-benchmark, --task-fixed\n");
                printf("Compatibility: --single and --once are accepted\n");
                printf("Usage (legacy): %s [-s|--single] [-d|--double]\n",argv[0]);
                return 0;
            }
        }
    } else {
        for(int i=1;i<argc;i++){
            ...
            else if(strcmp(argv[i],"-d")==0||strcmp(argv[i],"--double")==0) double_pass=1;
            else if(strcmp(argv[i],"-h")==0||strcmp(argv[i],"--help")==0){
                printf("Usage: %s [OPTIONS]\n"
                       "  -s, --single   Single cascade (1 forward pass)\n"
                       "  -d, --double   Double cascade (2 passes) [default]\n"
                       "  -h, --help     This help\n",argv[0]); return 0;
            }
        }
    }

    uint8_t key[KEY_SIZE]={0}; RAND_bytes(key,KEY_SIZE);
    build_op_table(key);

    if(use_named_cli){
        if(task<0) task=0;
        switch(task){
            case 0: return run_encrypt_task_cli(key,mode,double_pass,data_kb,text_arg);
            case 1: run_analysis_task_cli(key,mode,double_pass,data_kb); return 0;
            case 2: run_self_tests(key,mode,double_pass); return 0;
            case 3: benchmark(key,mode,double_pass); return 0;
            case 4: benchmark_fixed_key(key); return 0;
            default: break;
        }
    }

    printf("\n"
           "  WBC2-CASCADE-8  [8 rounds × composed_perm(%d ops) → XOR → S-box → rotate]\n"
           "  Composed perm: %d key bytes → %d op_ids → 1 lookup table per round\n"
           "  S-box: SHA-256(key) → MT19937 → Fisher-Yates {0..255}\n"
           "  cascade=%s\n\n",
           PERMS_PER_ROUND, PERMS_PER_ROUND, PERMS_PER_ROUND,
    ...
    while(running){
        printf("  ┌─ Menu ─────────────────────────────────────────────────────┐\n"
               ...
               "  │  4. Fixed-key benchmark (composed table, no cascade)       │\n"
        ...
        fflush(stdout);
        char buf[8]={0}; if(!fgets(buf,sizeof(buf),stdin)) break;
        int choice=atoi(buf);
        switch(choice){
        case 1:{
            ...
            tbuf[strcspn(tbuf,"\r\n")]=0;
            int bsz=auto_block_size((int)strlen(tbuf));
            size_t enc_len=0,dec_len=0; uint8_t *iv=NULL; size_t ivl=0;
            uint8_t *enc=cascade_encrypt(key,(uint8_t*)tbuf,strlen(tbuf),mode,NULL,bsz,&iv,&ivl,double_pass,&enc_len);
            uint8_t *dec=cascade_decrypt(key,enc,enc_len,mode,iv,bsz,double_pass,&dec_len);
        ...
        case 5:{
            ...
            mode=select_mode();
            int bsz=auto_block_size((int)tlen);
            size_t enc_len=0; uint8_t *iv=NULL; size_t ivl=0; uint8_t *enc=NULL;
            if(mode==MODE_WBC_CTR_HMAC)
                enc=cascade_encrypt_ctr_hmac(key,tdata,tlen,bsz,NULL,&enc_len);
            else
                enc=cascade_encrypt(key,tdata,tlen,mode,NULL,bsz,&iv,&ivl,double_pass,&enc_len);
        ...
        case 6: double_pass=!double_pass; g_comp_valid=0; printf("  Cascade: %s\n",double_pass?"DOUBLE":"SINGLE"); break;
        case 7: running=0; break;
        default: printf("  Unknown choice\n"); break;
        }
    }
    free(g_perms_buf); free(g_comp_fwd); free(g_comp_inv);
    printf("  Bye.\n"); return 0;
}
