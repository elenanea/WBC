/*
 * WBC1-CASCADE-NEW01 -- фиксированная таблица 127 операций + ключевой порядок
 * ============================================================================
 ...
 *
 * Build:
 *   gcc -O3 -march=native -o wbc1_cascade_new01 wbc1_cascade_new01.c -lssl -lcrypto -lm
 */
...
#define MAGIC_LEN    4
#define VERSION      0x01
#define VERSION_CASC 0x05   /* new version tag for fixed op-table variant */
...
static void apply_operation(Cube *cube, int op_id, int inverse) {
    ...
    switch (op) {
        case 95: xor_chain_forward(buf, n, inverse); return;
        case 96: xor_chain_backward(buf, n, inverse); return;
        case 97:
            if (!inverse) for (int i = 2; i < n; i += 2) buf[i] ^= buf[i - 2];
            else for (int i = n - ((n % 2) ? 1 : 2); i >= 2; i -= 2) buf[i] ^= buf[i - 2];
            return;
        case 98:
            if (!inverse) for (int i = 3; i < n; i += 2) buf[i] ^= buf[i - 2];
            else for (int i = n - ((n % 2) ? 2 : 1); i >= 3; i -= 2) buf[i] ^= buf[i - 2];
            return;
        case 99:
            if (!inverse) for (int i = 1; i < n; i += 2) buf[i] ^= buf[i - 1];
            else for (int i = ((n - 1) | 1); i >= 1; i -= 2) if (i < n) buf[i] ^= buf[i - 1];
            return;
        case 100:
            for (int i = 0; i < n / 2; i++) buf[i] ^= buf[n - 1 - i];
            return;
        case 101: xor_rows(buf, dim, inverse); return;
        case 102: xor_cols(buf, dim, inverse); return;
        case 103: xor_depth(buf, dim, inverse); return;
        case 104:
            if (!inverse) for (int i = 1; i < dim; i++) buf[idx3(dim, i, i, i)] ^= buf[idx3(dim, i - 1, i - 1, i - 1)];
            else for (int i = dim - 1; i >= 1; i--) buf[idx3(dim, i, i, i)] ^= buf[idx3(dim, i - 1, i - 1, i - 1)];
            return;
        case 105:
            for (int i = 0; i < n / 2; i++) buf[i + n / 2] ^= rotate_left(buf[i], 1);
            return;
        case 106:
            for (int i = 0; i < n / 2; i++) buf[i] ^= rotate_right(buf[i + n / 2], 1);
            return;
        case 107:
        ...
        case 112: {
            ...
            if (q > 0) {
                uint8_t tmp[n];
                memcpy(tmp, buf, n);
                memcpy(buf + 0 * q, tmp + 2 * q, q);
                memcpy(buf + 1 * q, tmp + 3 * q, q);
                memcpy(buf + 2 * q, tmp + 0 * q, q);
                memcpy(buf + 3 * q, tmp + 1 * q, q);
            }
            return;
        }
        case 113:
            for (int i = 0; i < dim / 2; i++)
                for (int k = 0; k < dim; k++)
                    for (int d = 0; d < dim; d++) {
                        int a = idx3(dim, d, i, k), b = idx3(dim, d, dim - 1 - i, k);
        ...
        case 114:
            for (int i = 0; i < dim / 2; i++)
                for (int j = 0; j < dim; j++)
                    for (int d = 0; d < dim; d++) {
                        int a = idx3(dim, d, j, i), b = idx3(dim, d, j, dim - 1 - i);
                        uint8_t t = buf[a]; buf[a] = buf[b]; buf[b] = t;
                    }
            return;
        case 115:
            for (int i = 0; i < dim / 2; i++)
                for (int j = 0; j < dim; j++)
                    for (int k = 0; k < dim; k++) {
                        int a = idx3(dim, i, j, k), b = idx3(dim, dim - 1 - i, j, k);
                        uint8_t t = buf[a]; buf[a] = buf[b]; buf[b] = t;
                    }
            return;
        case 116:
        ...
        case 121: case 122: case 123: case 124:
            op_feistel(buf, n, (op - 117) + 1, inverse, 0);
            return;
        case 125:
            op_feistel(buf, n, 3, inverse, 1);
            return;
        case 126:
            op_feistel(buf, n, 2, inverse, 2);
            return;
        default:
            return;
    }
...
/* ── block cipher: последовательные 32 раунда ───────────────────────────── */
...
static void encrypt_block(const uint8_t *key_mat, const uint8_t *block,
    ...
    /* 2. 32 rounds: permutation + rotation */
...
static void decrypt_block(const uint8_t *key_mat, const uint8_t *block,
    ...
    for (int i = 0; i < N; i++) buf[i] = block[i] ^ key_mat[i % KEY_SIZE];
    /* 2. 32 rounds in reverse: undo rotation + inverse permutation */
    for (int r = ROUNDS - 1; r >= 0; r--) {
        int op_id = key_mat[r] % NUM_OPS;
        Cube c = { buf, dim_for(N) };
        bitwise_rotate_cube(&c, op_id & 7, 0); /* left = undo right */
        apply_operation(&c, op_id, 1);         /* inverse perm */
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║     WBC1-CASCADE-NEW01 (32 раунда: таблица 127 операций) ║\n");
    printf("╠══════════════════════════════════════════════════════════╣\n");
    printf("║  Каскад:  %-46s ║\n",
