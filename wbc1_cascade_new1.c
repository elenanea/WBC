/*
 * WBC1-CASCADE-NEW1 -- упрощённая версия
 * =======================================
 * Отличия от wbc1_cascade_new.c:
 *
 * Оставлены ТОЛЬКО три операции в encrypt_block:
 *   1. Перестановка куба  — apply_operation(op_id)
 *   2. Циклический сдвиг  — bitwise_rotate_cube(op_id & 7, right)
 *   3. Каскад             — derive_cascade_key() между блоками
 *
 * Удалено:
 *   - mix_cube / inv_mix_cube  (XOR-диффузия)
 *   - XOR с раундовым ключом
 *   - Предвычисление таблицы раундовых ключей rk[ROUNDS][block_size]
 *
 * Это даёт:
 *   - Более простой для анализа алгоритм
 *   - Быстрее (меньше операций на раунд)
 *   - Меньший avalanche effect внутри одного блока (нет key-XOR и mix),
 *     но cascade-эффект между блоками сохраняется
 *
 * Build:
 *   gcc -O2 -o wbc1_cascade_new1 wbc1_cascade_new1.c -lssl -lcrypto -lm
 */
...
#define MAGIC_LEN    4
#define VERSION      0x01
#define VERSION_CASC 0x04   /* new version tag to distinguish from wbc1_cascade_new */
...
/* ── SHA-256/512 wrappers (used only at init/setup) ──────────────────────── */
 ...
 *
 * Ключевая идея: все 32 раунда (перестановка + побитовый сдвиг) сводятся к:
 *   out[i] = rotate_right(in[composed_fwd[i]], total_rot)
 *
 * Доказательство: rotate_right одинаково для всех позиций ⟹ коммутирует с
 * перестановками ⟹ можно вынести из цикла и накапливать: Σkᵣ mod 8.
 *
 * Кэш:
 *   • cascade ON  — ключ меняется каждый блок, кэш промахивается, composed
 *                   перестраивается за O(32·N). Overhead: ~3% vs оригинала.
 *   • cascade OFF — ключ константен, кэш всегда попадает после 1-го блока.
 *                   Применение = O(N). Ускорение ≈ 32×.
 * ──────────────────────────────────────────────────────────────────────────*/
static uint16_t *g_comp_fwd      = NULL;
static uint16_t *g_comp_inv      = NULL;
static uint8_t   g_comp_rot      = 0;
static uint8_t   g_comp_key[KEY_SIZE] = {0};
static int       g_comp_N        = -1;

static void build_composed(const uint8_t key_mat[KEY_SIZE], int N) {
    /* Reallocate if block size changed */
    if (g_comp_N != N) {
        free(g_comp_fwd);
        free(g_comp_inv);
        g_comp_fwd = (uint16_t *)malloc((size_t)N * sizeof(uint16_t));
        g_comp_inv = (uint16_t *)malloc((size_t)N * sizeof(uint16_t));
        g_comp_N   = N;
        g_comp_key[0] ^= 0xFF; /* invalidate key cache */
    }
    /* Cache hit: composed permutation already built for this key */
    if (memcmp(g_comp_key, key_mat, KEY_SIZE) == 0) return;

    /* Build composed permutation iteratively:
     *   new_composed[i] = old_composed[perm_r[i]]
     * After all rounds: composed_fwd[i] = index of the source byte for position i */
    uint16_t tmp[N];
    for (int i = 0; i < N; i++) g_comp_fwd[i] = (uint16_t)i;  /* identity */
    g_comp_rot = 0;
    for (int r = 0; r < ROUNDS; r++) {
        int op_id          = key_mat[r] % NUM_OPS;
        g_comp_rot         = (uint8_t)((g_comp_rot + (op_id & 7)) % 8);
        const uint16_t *pr = g_perms_buf + (size_t)op_id * N;
        for (int i = 0; i < N; i++) tmp[i] = g_comp_fwd[pr[i]];
        memcpy(g_comp_fwd, tmp, (size_t)N * sizeof(uint16_t));
    }
    /* Inverse: g_comp_inv[g_comp_fwd[i]] = i */
...
/* ==========================================================================
 * Раунд r применяет:  out[i] = rotate_right(in[perm_r[i]], k_r)
 *
 * Для двух раундов:
 *   x₁[i] = rotate(x₀[P₁[i]], k₁)
 *   x₂[i] = rotate(x₁[P₂[i]], k₂)
 *          = rotate(rotate(x₀[P₁[P₂[i]]], k₁), k₂)
 *          = rotate(x₀[P₁[P₂[i]]], k₁ + k₂)
 *
 * Вращение КОММУТИРУЕТ, потому что применяется ко ВСЕМ позициям одинаково
 * в каждом раунде — поэтому оно «проваливается» сквозь перестановки.
 *
 * Для 32 раундов:
 *   out[i] = rotate_right(x₀[composed_fwd[i]], total_rot)
 *
 * Используется кэш build_composed() — см. выше.
 * ========================================================================== */

static void encrypt_block(const uint8_t *key_mat, const uint8_t *block,
                           int block_size, uint8_t *out) {
    int N = block_size;
    build_composed(key_mat, N);               /* no-op if key unchanged */

    uint8_t tmp[N];
    /* 1. Initial key whitening */
    for (int i = 0; i < N; i++) tmp[i] = block[i] ^ key_mat[i % KEY_SIZE];
    /* 2. One-pass: composed permutation + total rotation */
    if (g_comp_rot)
        for (int i = 0; i < N; i++) out[i] = rotate_right(tmp[g_comp_fwd[i]], g_comp_rot);
    else
...
static void decrypt_block(const uint8_t *key_mat, const uint8_t *block,
                           int block_size, uint8_t *out) {
    int N = block_size;
    build_composed(key_mat, N);

    uint8_t tmp[N];
    /* 1. Undo final whitening */
    for (int i = 0; i < N; i++) tmp[i] = block[i] ^ key_mat[i % KEY_SIZE];
    /* 2. One-pass: inverse composed permutation + undo total rotation */
    if (g_comp_rot)
        for (int i = 0; i < N; i++) out[i] = rotate_left(tmp[g_comp_inv[i]], g_comp_rot);
    else
        for (int i = 0; i < N; i++) out[i] = tmp[g_comp_inv[i]];
    /* 3. Undo initial whitening */
    for (int i = 0; i < N; i++) out[i] ^= key_mat[i % KEY_SIZE];
}
...
static uint8_t *cascade_encrypt_ctr_hmac(const uint8_t key[KEY_SIZE],
    ...
    for (size_t b = 0; b < n_blocks; b++) {
        ...
        /* Fill with current cascade key (rk) — rk changes each block,
         * ensuring unique non-zero content and no periodic structure. */
...
static void benchmark(const uint8_t key[32], EncMode mode, int double_pass) {
    ...
    if (cpu_mhz > 0) printf("  CPU: %.0f MHz\n", cpu_mhz);
    printf("  %10s  %10s  %12s  %12s  %8s  %8s  Integrity\n",
           "Size (KB)","Enc (s)","Enc (KB/s)","Dec (KB/s)","Enc CPB","Dec CPB");
    printf("  %s\n","----------------------------------------------------------------------------");
    ...
    for (int si=0; si<ns; si++) {
        ...
        double enc_cpb = (cpu_mhz>0 && enc_spd>0) ? cpu_mhz*1e3/enc_spd : 0.0;
        double dec_cpb = (cpu_mhz>0 && dec_spd>0) ? cpu_mhz*1e3/dec_spd : 0.0;
        printf("  %10.2f  %10.5f  %12.2f  %12.2f  %8.2f  %8.2f  %s\n",
               (double)sz/1024.0, enc_avg, enc_spd, dec_spd, enc_cpb, dec_cpb, all_ok?"OK":"FAIL");
        free(data);
    }
    printf("\n");
}

/* ── fixed-key benchmark (показывает выигрыш кэша составной перестановки) ── */

static void benchmark_fixed_key(const uint8_t key[KEY_SIZE]) {
    /* Тест без каскада: тот же ключ для каждого блока.
       Первый блок: промах кэша (build_composed).
       Все последующие: попадание кэша → одна перестановка вместо 32. */
    static const int DATA_MB = 32;
    int N = 1331; /* блок d=11 */
    ...
    build_op_table(key); /* инициализация g_ops */
    build_perm_table(N); /* инициализация g_perms_buf */

    /* --- WITHOUT cache: сбрасываем кэш перед каждым блоком --- */
    g_comp_N = -1; /* инвалидировать кэш */
    struct timespec t0, t1;
    clock_gettime(CLOCK_MONOTONIC, &t0);
    for (size_t b = 0; b < nblocks; b++) {
        g_comp_key[0] ^= 0xFF; /* принудительный промах */
        encrypt_block(key, data + b * N, N, enc + b * N);
    }
    clock_gettime(CLOCK_MONOTONIC, &t1);
    double t_nocache = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) / 1e9;

    /* --- WITH cache: ключ не меняется → кэш попадает со 2-го блока --- */
    g_comp_N = -1;
    clock_gettime(CLOCK_MONOTONIC, &t0);
    for (size_t b = 0; b < nblocks; b++) {
        encrypt_block(key, data + b * N, N, enc + b * N);
    ...
    clock_gettime(CLOCK_MONOTONIC, &t1);
    double t_cached = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) / 1e9;

    /* decrypt с кэшем для проверки */
    for (size_t b = 0; b < nblocks; b++)
        decrypt_block(key, enc + b * N, N, dec + b * N);
    int ok = (memcmp(data, dec, total) == 0);

    double spd_nc = (total / 1024.0 / 1024.0) / (t_nocache > 0 ? t_nocache : 1e-9);
    double spd_c  = (total / 1024.0 / 1024.0) / (t_cached  > 0 ? t_cached  : 1e-9);

    /* --- Avalanche: single block, fixed key (ECB-style) ---
     * Flip each bit of plaintext → count changed bits in ciphertext.
     * Pure permutation+rotation: ровно 1 бит меняется при 1 перевёрнутом бите.
     * С mix_cube/key-XOR было бы ~50%. */
    uint8_t blk0[N], enc0[N], enc1[N], blk1[N];
    RAND_bytes(blk0, N);
    encrypt_block(key, blk0, N, enc0);

    long long avl_flips = 0;
    int probe = (N * 8 < 2000) ? N * 8 : 2000; /* до 2000 случайных битовых флипов */
    for (int i = 0; i < probe; i++) {
        memcpy(blk1, blk0, N);
        blk1[i / 8] ^= (uint8_t)(1u << (i % 8));
        encrypt_block(key, blk1, N, enc1);
        for (int j = 0; j < N; j++) avl_flips += __builtin_popcount(enc0[j] ^ enc1[j]);
    }
    double avl_pct = 100.0 * (double)avl_flips / ((double)probe * (double)N * 8.0);

    /* --- Cascade avalanche: 1 бит меняется в блоке 0, считаем diff во всех последующих --- */
    int cblocks = 16; /* небольшое число блоков */
    uint8_t *cdata  = (uint8_t *)malloc((size_t)cblocks * N);
    uint8_t *cenc0  = (uint8_t *)malloc((size_t)cblocks * N);
    uint8_t *cenc1  = (uint8_t *)malloc((size_t)cblocks * N);
    RAND_bytes(cdata, cblocks * N);
    uint8_t crk0[KEY_SIZE], crk1[KEY_SIZE];
    memcpy(crk0, key, KEY_SIZE); memcpy(crk1, key, KEY_SIZE);
    uint8_t ctmp0[N], ctmp1[N];
    /* Шифруем поток из cblocks блоков с derive_cascade_key между ними */
    for (int b = 0; b < cblocks; b++) {
        encrypt_block(crk0, cdata + b*N, N, ctmp0);
        encrypt_block(crk1, cdata + b*N, N, ctmp1);
        if (b == 0) ctmp1[0] ^= 0x01;  /* флипаем 1 бит в шифртексте блока 0 */
        memcpy(cenc0 + b*N, ctmp0, N);
        memcpy(cenc1 + b*N, ctmp1, N);
        derive_cascade_key(cenc0 + b*N, N, crk0);
        derive_cascade_key(cenc1 + b*N, N, crk1);
    }
    long long casc_diff[16] = {0};
    for (int b = 0; b < cblocks; b++)
        for (int j = 0; j < N; j++)
            casc_diff[b] += __builtin_popcount(cenc0[b*N+j] ^ cenc1[b*N+j]);
    free(cdata); free(cenc0); free(cenc1);

    /* --- Вывод --- */
    printf("\n  Fixed-key benchmark  (%d MB, блок %d байт, %zu блоков)\n",
           DATA_MB, N, nblocks);
    double cpu_mhz = read_cpu_mhz();
    if (cpu_mhz > 0) printf("  CPU: %.0f MHz\n", cpu_mhz);
    double enc_cpb_nc = (cpu_mhz > 0 && spd_nc > 0) ? cpu_mhz * 1e3 / spd_nc : 0.0;
    double enc_cpb_c  = (cpu_mhz > 0 && spd_c  > 0) ? cpu_mhz * 1e3 / spd_c  : 0.0;
    printf("  %-35s %8.1f MB/s  %6.2f cpb\n", "Без кэша (промах каждый блок):", spd_nc, enc_cpb_nc);
    printf("  %-35s %8.1f MB/s  %6.2f cpb\n", "С кэшем  (попадание 2..N-го):", spd_c, enc_cpb_c);
    printf("  %-35s %8.1fx\n",     "Ускорение:", spd_c / (spd_nc > 0 ? spd_nc : 1.0));
    printf("  Integrity check: %s\n", ok ? "OK" : "FAIL");

    printf("\n  --- Avalanche (фиксированный ключ, ECB) ---\n");
    printf("  1 бит в plaintext → изменилось в ciphertext: %.4f%%  (идеал ~50%%)\n", avl_pct);
    printf("  Объяснение: permutation+rotation = биекция,\n"
           "              каждый входной бит отображается ровно в 1 выходной бит.\n"
           "              Без mix_cube нелинейности нет → лавина ~1/(N*8).\n");

    printf("\n  --- Cascade avalanche (1 бит flip в блоке 0 → diff в блоках 0..%d) ---\n", cblocks-1);
    printf("  Блок  |  Изменено бит (из %d)\n", N*8);
    for (int b = 0; b < cblocks; b++)
        printf("  [%2d]  |  %5lld  (%.1f%%)\n", b, casc_diff[b], 100.0*casc_diff[b]/(N*8));
    printf("\n");
...
int main(int argc, char *argv[]) {
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║      WBC1-CASCADE-NEW1  (только перестановка + сдвиг)   ║\n");
    printf("╠══════════════════════════════════════════════════════════╣\n");
    printf("║  Каскад:  %-46s ║\n",
           double_pass ? "ДВОЙНОЙ  (-d/--double)  [прямой + обратный]"
                       : "ОДИНОЧНЫЙ (-s/--single)  [только прямой]");
    printf("║  Раунды:  32  |  Операции: перестановка + сдвиг         ║\n");
    printf("╚══════════════════════════════════════════════════════════╝\n");
    ...
    while (1) {
        printf("\n"
               ...
               "4. Fixed-key benchmark (кэш составной перестановки)\n"
