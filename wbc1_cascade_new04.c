/*
 * WBC1-CASCADE-NEW04 -- упрощенная таблица перестановок
 * =====================================================
 ...
 *
 * В отличие от wbc1_cascade_new1.c (с кэшем составной перестановки):
 *   - 32 раунда применяются последовательно в цикле
 *   - Нет кэша: каждый блок делает 32 × apply_operation + bitwise_rotate
 *   - Более понятная структура раундов
 *
 * Build:
 *   gcc -O3 -march=native -o wbc1_cascade_new04 wbc1_cascade_new04.c -lssl -lcrypto -lm
 */
...
#define MAGIC_LEN    4
#define VERSION      0x01
#define VERSION_CASC 0x08   /* NEW04: simplified permutation table */
...
/* ── fixed operation table + key-derived order ──────────────────────────── */

static uint8_t g_op_order[NUM_OPS];
...
static void build_op_table(const uint8_t key[32]) {
    ...
    for (int i=0;i<NUM_OPS;i++) g_op_order[i]=(uint8_t)order[i];
}

/* ── operation execution (fixed table 0..126) ───────────────────────────── */
static uint16_t *g_perms_buf = NULL;
static int      g_perm_bs   = -1;
static uint8_t  g_op_use_table[NUM_OPS];

static void rotate_bytes_block(uint8_t *buf, int n, int right, int total) {
    if (total <= 1) return;
    n %= total;
    if (n < 0) n += total;
    if (n == 0) return;
    int s = right ? (total - n) : n;
    uint8_t tmp[total];
    for (int i = 0; i < total; i++) tmp[i] = buf[(i + s) % total];
    memcpy(buf, tmp, total);
}

static void permute_interleave_halves(uint8_t *buf, int n) {
    if (n < 2) return;
    uint8_t tmp[n];
    int lsz = n / 2;
    int rsz = n - lsz;
    ...
    for (int i = 0; i < lsz || i < rsz; i++) {
        if (i < lsz) tmp[p++] = buf[i];
        if (i < rsz) tmp[p++] = buf[lsz + i];
    }
    memcpy(buf, tmp, n);
}

static void permute_deinterleave_halves(uint8_t *buf, int n) {
    if (n < 2) return;
    uint8_t tmp[n];
    int lsz = n / 2;
    int rsz = n - lsz;
    int p = 0;
    for (int i = 0; i < lsz || i < rsz; i++) {
        if (i < lsz) tmp[i] = buf[p++];
        if (i < rsz) tmp[lsz + i] = buf[p++];
    }
    memcpy(buf, tmp, n);
}

static void sparse_chain_forward(uint8_t *buf, int n) {
    for (int i = 2; i < n; i += 2) buf[i] ^= buf[i - 2];
}

static void sparse_unchain_forward(uint8_t *buf, int n) {
    int start = (n % 2 == 0) ? (n - 2) : (n - 1);
    for (int i = start; i >= 2; i -= 2) buf[i] ^= buf[i - 2];
}

static void sparse_chain_backward(uint8_t *buf, int n) {
    for (int i = n - 3; i >= 0; i -= 2) buf[i] ^= buf[i + 2];
}

static void sparse_unchain_backward(uint8_t *buf, int n) {
    int start = (n % 2 == 0) ? 1 : 0;
    for (int i = start; i + 2 < n; i += 2) buf[i] ^= buf[i + 2];
}

static void apply_operation_direct(Cube *cube, int op_id, int inverse);

static int op_is_positional(int effective_op) {
    (void)effective_op;
    return 1;
}

#define MAX_OP_SEQ 3
static uint8_t g_op_seq_len[NUM_OPS];
static uint8_t g_op_seq[NUM_OPS][MAX_OP_SEQ];
static int     g_op_catalog_inited = 0;

static int inv_turn(int t) {
    return (t == 0) ? 1 : (t == 1) ? 0 : 2;
}

static void face_axis_pos(int face, int dim, int depth, int *axis, int *pos) {
    if (depth < 0) depth = 0;
    if (depth >= dim) depth = dim - 1;
    switch (face) {
        case 0: *axis = 0; *pos = depth; break;          /* U */
        case 1: *axis = 0; *pos = dim - 1 - depth; break;/* D */
        case 2: *axis = 1; *pos = depth; break;          /* L */
        case 3: *axis = 1; *pos = dim - 1 - depth; break;/* R */
        case 4: *axis = 2; *pos = depth; break;          /* F */
        default:*axis = 2; *pos = dim - 1 - depth; break;/* B */
    }
}

static void apply_basic_move(Cube *cube, int move_id, int inverse) {
    static const int dir_map[] = {1, -1, 2};
    int dim = cube->dim;

    if (move_id >= 0 && move_id <= 17) {
        int face = move_id / 3;
        int t = move_id % 3;
        int k = dir_map[t];
        if (inverse && k != 2) k = -k;
        int axis, pos;
        face_axis_pos(face, dim, 0, &axis, &pos);
        rotate_slice(cube, axis, pos, k);
        return;
    }

    if (move_id >= 18 && move_id <= 35) {
        int local = move_id - 18;
        int face = local / 3;
        int t = local % 3;
        int k = dir_map[t];
        if (inverse && k != 2) k = -k;
        int axis, pos;
        face_axis_pos(face, dim, 1, &axis, &pos);
        rotate_slice(cube, axis, pos, k);
        return;
    }

    if (move_id >= 36 && move_id <= 53) {
        int local = move_id - 36;
        int face = local / 3;
        int t = local % 3;
        int k = dir_map[t];
        if (inverse && k != 2) k = -k;
        int axis, pos;
        face_axis_pos(face, dim, 2, &axis, &pos);
        rotate_slice(cube, axis, pos, k);
        return;
    }

    if (move_id >= 54 && move_id <= 71) {
        int local = move_id - 54;
        int face = local / 3;
        int t = local % 3;
        int k = dir_map[t];
        if (inverse && k != 2) k = -k;
        int axis, pos0, pos1;
        face_axis_pos(face, dim, 0, &axis, &pos0);
        face_axis_pos(face, dim, 1, &axis, &pos1);
        rotate_slice(cube, axis, pos0, k);
        if (pos1 != pos0) rotate_slice(cube, axis, pos1, k);
        return;
    }

    if (move_id >= 72 && move_id <= 80) {
        int local = move_id - 72;
        int axis = local / 3;
        int t = local % 3;
        int k = dir_map[t];
        if (inverse && k != 2) k = -k;
...
static void build_op_catalog(void) {
    if (g_op_catalog_inited) return;

    for (int i = 0; i < NUM_OPS; i++) {
        g_op_seq_len[i] = 1;
        g_op_seq[i][0] = (uint8_t)(i % 81);
        g_op_seq[i][1] = 0;
        g_op_seq[i][2] = 0;
    }
    ...
    for (int i = 0; i <= 80; i++) {
        g_op_seq_len[i] = 1;
        g_op_seq[i][0] = (uint8_t)i;
    }

    static const uint8_t pair_ops[16][2] = {
        {0, 9}, {12, 3}, {6, 15}, {9, 4},
        {54, 60}, {57, 66}, {72, 0}, {75, 9},
        {18, 30}, {24, 42}, {36, 48}, {21, 33},
        {60, 6}, {66, 12}, {78, 15}, {30, 54}
    };
    for (int i = 0; i < 16; i++) {
        int op = 81 + i;
        g_op_seq_len[op] = 2;
        g_op_seq[op][0] = pair_ops[i][0];
        g_op_seq[op][1] = pair_ops[i][1];
    }

    static const uint8_t triple_ops[30][3] = {
        /* A B A' with different axes (cheap but non-local). */
        {0, 9, 1}, {12, 6, 13}, {3, 15, 4}, {9, 12, 10},
        {6, 3, 7}, {15, 0, 16},

        /* Wide-layer sandwiches. */
        {54, 9, 55}, {57, 12, 58}, {60, 6, 61}, {63, 15, 64},
        {66, 0, 67}, {69, 3, 70},

        /* Cube-rotation conjugations: R B R^-1 in rotated frame. */
        {72, 0, 73}, {72, 12, 73}, {75, 9, 76}, {75, 3, 76},
        {78, 6, 79}, {78, 15, 79},

        /* Inner-layer variants for depth mixing. */
        {18, 30, 19}, {24, 42, 25}, {36, 48, 37}, {21, 33, 22},

        /* Cross-family mixes (outer + wide + rotated). */
        {0, 60, 1}, {12, 54, 13}, {9, 66, 10}, {6, 57, 7},
        {30, 54, 31}, {42, 60, 43}, {48, 57, 49}, {33, 66, 34}
    };
    for (int i = 0; i < 30; i++) {
        int op = 97 + i;
        g_op_seq_len[op] = 3;
        g_op_seq[op][0] = triple_ops[i][0];
        g_op_seq[op][1] = triple_ops[i][1];
        g_op_seq[op][2] = triple_ops[i][2];
    }

    g_op_catalog_inited = 1;
}

static void xor_mask_pattern(uint8_t *buf, int n, uint8_t mul, uint8_t add) {
    for (int i = 0; i < n; i++)
        buf[i] ^= (uint8_t)((uint8_t)(i * mul) + add);
}

static void xor_chain_forward(uint8_t *buf, int total, int inverse) {
    if (!inverse) {
        for (int i = 1; i < total; i++) buf[i] ^= buf[i - 1];
    } else {
        for (int i = total - 1; i >= 1; i--) buf[i] ^= buf[i - 1];
    }
}

static void xor_chain_backward(uint8_t *buf, int total, int inverse) {
    if (!inverse) {
        for (int i = total - 2; i >= 0; i--) buf[i] ^= buf[i + 1];
    } else {
        for (int i = 0; i < total - 1; i++) buf[i] ^= buf[i + 1];
    }
}

static void xor_rows(uint8_t *buf, int dim, int inverse) {
    if (!inverse) {
        for (int i = 0; i < dim; i++)
            for (int j = 0; j < dim; j++)
                for (int k = 1; k < dim; k++)
                    buf[idx3(dim, i, j, k)] ^= buf[idx3(dim, i, j, k - 1)];
    } else {
        for (int i = 0; i < dim; i++)
            for (int j = 0; j < dim; j++)
...
static void xor_cols(uint8_t *buf, int dim, int inverse) {
    if (!inverse) {
        for (int i = 0; i < dim; i++)
            for (int k = 0; k < dim; k++)
                for (int j = 1; j < dim; j++)
                    buf[idx3(dim, i, j, k)] ^= buf[idx3(dim, i, j - 1, k)];
    } else {
        for (int i = 0; i < dim; i++)
            for (int k = 0; k < dim; k++)
...
static void xor_depth(uint8_t *buf, int dim, int inverse) {
    if (!inverse) {
        for (int j = 0; j < dim; j++)
            for (int k = 0; k < dim; k++)
                for (int i = 1; i < dim; i++)
                    buf[idx3(dim, i, j, k)] ^= buf[idx3(dim, i - 1, j, k)];
    } else {
        for (int j = 0; j < dim; j++)
            for (int k = 0; k < dim; k++)
...
static void op_feistel(uint8_t *buf, int total, int rot, int inverse, int mode) {
    if (total < 2) return;
    int lsz = total / 2;
    int rsz = total - lsz;
    uint8_t tmp[total];
    ...
    if (!inverse) {
        for (int i = 0; i < rsz; i++) tmp[i] = buf[lsz + i];
        for (int i = 0; i < lsz; i++) {
            int ridx;
            if (mode == 1) ridx = rsz - 1 - (i % rsz);
            else if (mode == 2) ridx = (i ^ (i >> 1)) % rsz;
            else ridx = i % rsz;
            uint8_t r = buf[lsz + ridx];
            uint8_t f = rotate_left((uint8_t)(r + (uint8_t)(31 * i + 7 * mode)), rot & 7);
            tmp[rsz + i] = (uint8_t)(buf[i] ^ f);
        }
    } else {
        for (int i = 0; i < rsz; i++) tmp[lsz + i] = buf[i];
        for (int i = 0; i < lsz; i++) {
            int ridx;
            if (mode == 1) ridx = rsz - 1 - (i % rsz);
            else if (mode == 2) ridx = (i ^ (i >> 1)) % rsz;
            ...
            uint8_t r0 = buf[ridx];
            uint8_t f = rotate_left((uint8_t)(r0 + (uint8_t)(31 * i + 7 * mode)), rot & 7);
            tmp[i] = (uint8_t)(buf[rsz + i] ^ f);
        }
    }
...
static void build_perm_table(int block_size) {
    ...
    for (int op = 0; op < NUM_OPS; op++) {
        int effective_op = g_op_order[op];
        if (!op_is_positional(effective_op)) {
            g_op_use_table[op] = 0;
            continue;
        }
...
static void apply_operation(Cube *cube, int op_id, int inverse) {
    ...
    if (!g_op_use_table[idx]) {
        apply_operation_direct(cube, idx, inverse);
        return;
    }
...
static void apply_operation_direct(Cube *cube, int op_id, int inverse) {
    int idx = ((op_id % NUM_OPS) + NUM_OPS) % NUM_OPS;
    int op  = g_op_order[idx];

    if (!g_op_catalog_inited) build_op_catalog();
    int len = g_op_seq_len[op];

    if (!inverse) {
        for (int i = 0; i < len; i++)
            apply_basic_move(cube, g_op_seq[op][i], 0);
    } else {
        for (int i = len - 1; i >= 0; i--)
            apply_basic_move(cube, g_op_seq[op][i], 1);
    }
}
...
/* ── block cipher: operation -> XOR round key -> bitwise shift ─────────── */
...
static void encrypt_block(const uint8_t *key_mat, const uint8_t *block,
    ...
    /* 2. 32 rounds: op -> XOR round key -> bitwise rotate */
    for (int r = 0; r < ROUNDS; r++) {
        int op_id = key_mat[r] % NUM_OPS;
        int alpha = (key_mat[(r + 13) % KEY_SIZE] & 7) + 1;
        Cube c = { buf, dim_for(N) };
        apply_operation(&c, op_id, 0);

        for (int i = 0; i < N; i++)
            buf[i] ^= key_mat[(r + i) % KEY_SIZE];

        bitwise_rotate_cube(&c, alpha, 1);
    }
...
static void benchmark(const uint8_t key[32], EncMode mode, int double_pass) {
    ...
    for (int si=0; si<ns; si++) {
        ...
        printf("  %10.2f  %10.5f  %14.2f  %14.2f  %12.4f  %12.4f  %14.4f  %14.4f  %s\n",
               (double)sz/1024.0, enc_avg, enc_spd, dec_spd, enc_mbps, dec_mbps, enc_mbit, dec_mbit, all_ok?"OK":"FAIL");
...
/* ── fixed-key benchmark (throughput без каскада) ─────────────────────── */

static void benchmark_fixed_key(const uint8_t key[KEY_SIZE]) {
    /* Тест без каскада: тот же ключ для каждого блока, 32 раунда последовательно. */
    ...
    double spd_nc = (total / 1024.0 / 1024.0) / (t_enc > 0 ? t_enc : 1e-9);
    double spd_c  = (total / 1024.0 / 1024.0) / (t_dec > 0 ? t_dec : 1e-9);

    /* --- Вывод --- */
    printf("\n  Fixed-key benchmark  (%d MB, блок %d байт, %zu блоков, 32 раунда)\n",
           DATA_MB, N, nblocks);
    printf("  Encrypt: %8.1f MB/s\n", spd_nc);
    printf("  Decrypt: %8.1f MB/s\n", spd_c);
...
int main(int argc, char *argv[]) {
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║ WBC1-CASCADE-NEW04 (32 раунда: simplified perm table)     ║\n");
    printf("╠══════════════════════════════════════════════════════════╣\n");
    printf("║  Каскад:  %-46s ║\n",
