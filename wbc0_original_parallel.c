...
typedef struct {
    ...
    int shift_mode;  /* 0=baseline, 1=uniform key-byte-dependent, 2=alpha+beta compromise */
...
static uint8_t expand_round_key_byte(const uint8_t *round_key, int round_key_len, int index) {
    ...
    /* z_i = center ⊕ ROTL8(right, i mod 8) ⊕ ROTR8(left, (i+3) mod 8) ⊕ (29*i mod 256) */
...
static void build_round_schedule(WBC1OriginalCipher *cipher,
    ...
    for (int byte_idx = 0; byte_idx < round_key_len; byte_idx++) {
        ...
        /* j_i = mixed_byte mod 127 */
        op_ids[byte_idx] = mixed_byte % NUM_OPERATIONS;
        if (cipher->shift_mode == 2) {
            int alpha = byte_idx % 8;
            int beta = mixed_byte % 4;
            /* Compromise schedule: Δ_i = (alpha_i + beta_i) mod M, M=subblock_bits */
            shifts[byte_idx] = (alpha + beta) % subblock_bits;
        } else if (cipher->shift_mode == 1) {
            /* Alternative schedule: Δ_i = 1 + (m_i mod d), d>=1 */
            shifts[byte_idx] = 1 + (mixed_byte % cipher->cube_d);
        } else {
            /* Baseline schedule: Δ_i = d if MSB(m_i)=1, otherwise floor(d/2)+1 */
...
static void process_subblock_with_key(WBC1OriginalCipher *cipher,
    ...
    build_round_schedule(cipher, round_key, round_key_len, subblock_size, op_ids, shifts);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        if (rank == 0) {
            printf("Usage: %s <task> <key_size> <key_source> <block_size_bits> [mode] [data_size_kb] [seed] [avalanche_tests] [subblock_size_bytes] [shift_mode]\n", argv[0]);
            ...
            printf("  subblock_size_bytes: manual subblock size in bytes (optional, 0=auto, 1..block_size_bytes=override)\n");
            printf("  shift_mode: 0=baseline shift schedule, 1=uniform key-byte-based schedule, 2=compromise (alpha+beta) schedule (optional)\n");
            printf("\nExamples:\n");
            ...
            printf("  %s 1 256 0 512 1 1000 42 500 4 1  # Force 4-byte subblocks + alt shift mode\n", argv[0]);
    ...
    if (argc >= 10) {
        ...
        if (subblock_override_bytes < 0) subblock_override_bytes = 0;
    }
    if (argc >= 11) {
        shift_mode = atoi(argv[10]);
    ...
    if (task == 0) {
        ...
        if (rank == 0) {
            ...
            const char *shift_mode_name = "baseline";
            if (cipher.shift_mode == 1) {
                shift_mode_name = "uniform-key-byte";
            } else if (cipher.shift_mode == 2) {
                shift_mode_name = "compromise-alpha+beta";
            }
            printf("Shift mode / Режим сдвига: %d (%s)\n", cipher.shift_mode, shift_mode_name);
