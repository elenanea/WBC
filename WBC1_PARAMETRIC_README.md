...
**Важно:** Параметрическая версия примерно в **3 раза медленнее** кешированных версий.

**Причина:**
- PRF (SHA-256) вычисляется для каждой операции в каждом раунде
- 32 операции × 16 раундов = 512 вызовов SHA-256 на блок
- Vs. кешированные версии: операции предвычислены один раз

**Компромисс:**
- **Безопасность:** Ключезависимые операции, бесконечное пространство, устойчивость к криптоанализу
- **Скорость:** 3x медленнее из-за динамических вычислений

**Оптимизации:**
- Кеш ротаций сохранен (экономит побитовые операции)
- Параллелизация MPI работает (ускорение на многих ядрах)
- Возможна дополнительная оптимизация: кеширование θ для повторяющихся блоков
...
|--------|----------|------------------|-------------------|-----------------|
| wbc1_parallel | 127 фикс. | Нет | ⚡⚡⚡⚡⚡ (самая быстрая) | ⭐⭐⭐ |
| wbc1_parallel_cached_new | 127 фикс. | Нет | ⚡⚡⚡⚡ | ⭐⭐⭐⭐ |
| wbc1_parallel_cached_opti | 127 фикс. | Нет | ⚡⚡⚡⚡⚡ | ⭐⭐⭐⭐ |
| **wbc1_parallel_gen_cached** | **∞ генер.** | **Да** | **⚡⚡ (3x медленнее)** | **⭐⭐⭐⭐⭐ (максимум)** |

**Примечание:** Параметрическая версия медленнее из-за 512 вызовов SHA-256 на блок, но обеспечивает максимальную криптостойкость благодаря ключезависимым операциям.
