...
/* Extended benchmark similar to WBC1 output style (single-process core path) */
static void benchmark_core(WBC2OriginalCipher *cipher) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);

    static const int sizes_kb[] = {1, 10, 100, 1000, 10000};
    int ns = (int)(sizeof(sizes_kb) / sizeof(sizes_kb[0]));
    int block_size = cipher->block_size_bytes;
    ...
    double dec_mbps[5] = {0};
        ...
        if (rank == 0) {
         ...
         printf("  %10s  %10s  %10s  %10s  %10s  %9s\n",
             "Size (KB)", "Enc (s)", "Dec (s)", "Enc MB/s", "Dec MB/s", "Check");
         printf("  %s\n", "-----------------------------------------------------------------------");
        }

    for (int si = 0; si < ns; si++) {
        int size = sizes_kb[si] * 1024;
        ...
        if (rank == 0) {
            ...
            enc = (uint8_t *)malloc((size_t)padded_len);
            dec = (uint8_t *)malloc((size_t)padded_len);

            if (!plain || !plain_padded || !enc || !dec) {
                ok = 0;
            } else {
                memcpy(plain_padded, plain, size);
        ...
        if (ok) {
            ...
            if (rank == 0) free(dummy_cipher);
        }
        double t_enc_local = MPI_Wtime() - t0;
        double t_enc = 0.0;
        MPI_Reduce(&t_enc_local, &t_enc, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        ...
        if (ok && rank == 0 && enc) {
            ...
            uint8_t *dummy_plain = NULL;
            int dummy_plain_len = 0;
            parallel_original_decrypt(cipher, NULL, 0, &dummy_plain, &dummy_plain_len);
            if (rank == 0) free(dummy_plain);
        }
        double t_dec_local = MPI_Wtime() - t0;
        double t_dec = 0.0;
        MPI_Reduce(&t_dec_local, &t_dec, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

        if (rank == 0) {
            int check = (ok && dec && plain_padded && dec_len == padded_len && memcmp(plain_padded, dec, (size_t)padded_len) == 0);
            double enc_speed = (size / 1024.0 / 1024.0) / (t_enc > 0 ? t_enc : 1e-9);
            double dec_speed = (size / 1024.0 / 1024.0) / (t_dec > 0 ? t_dec : 1e-9);
            enc_mbps[si] = enc_speed;
            dec_mbps[si] = dec_speed;

            printf("  %10d  %10.5f  %10.5f  %10.2f  %10.2f  %9s\n",
                   sizes_kb[si], t_enc, t_dec, enc_speed, dec_speed, check ? "OK" : "FAIL");
        }
    ...
    if (rank == 0) {
        ...
        if (cpu_mhz > 0.0) {
            printf("\nCPB Estimation / Оценка cycles-per-byte (CPU: %.0f MHz):\n", cpu_mhz);
            printf("  %10s  %10s  %10s\n", "Size (KB)", "Enc CPB", "Dec CPB");
            printf("  %s\n", "------------------------------------------");
            for (int si = 0; si < ns; si++) {
                double ecpb = (enc_mbps[si] > 0.0) ? (cpu_mhz / enc_mbps[si]) : 0.0;
                double dcpb = (dec_mbps[si] > 0.0) ? (cpu_mhz / dec_mbps[si]) : 0.0;
                printf("  %10d  %10.2f  %10.2f\n", sizes_kb[si], ecpb, dcpb);
            }
        } else {
            printf("\nCPB Estimation: CPU frequency unavailable\n");
        }
        printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    }
}
...
int main(int argc, char *argv[]) {
    ...
    int num_rounds = 6;  /* default value */
    if (argc >= 8) {
