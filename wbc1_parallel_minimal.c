/*
 * WBC1 Block Cipher - Parallel Implementation with MPI
 * 
...
/* Algorithm modes */
#define MODE_SIMPLIFIED 0  /* 4 operations: 3x Rubik permutation + rotation (minimal enhanced) */
...
void wbc1_encrypt_block(WBC1Cipher *cipher, const uint8_t *plaintext, uint8_t *ciphertext) {
    ...
    for (int round = 0; round < cipher->num_rounds; round++) {
        if (cipher->algorithm_mode == MODE_FULL) {
            /* MODE 1: Full algorithm with 5 operations */
            ...
            /* MODE 0: Minimal enhanced - Multi-axis Rubik rotations */
            
            /* 1. Primary Rubik's cube rotation (X-axis) */
            int op_id1 = cipher->round_keys[round][0] % NUM_OPERATIONS;
            apply_operation(cipher, ciphertext, op_id1, 0);
            
            /* 2. Secondary Rubik's cube rotation (Y-axis) */
            int op_id2 = cipher->round_keys[round][1] % NUM_OPERATIONS;
            if (op_id2 != op_id1) {  /* Ensure different operations */
                apply_operation(cipher, ciphertext, op_id2, 0);
            }
            
            /* 3. Tertiary Rubik's cube rotation (Z-axis) */
            ...
            /* 4. Cyclic bitwise rotation */
            int shift = (cipher->block_size > 1) ? cipher->round_keys[round][3] % 8 : round % 8;
            cyclic_bitwise_rotate(ciphertext, cipher->block_size, shift, 0);
        }
    }
}
...
void wbc1_decrypt_block(WBC1Cipher *cipher, const uint8_t *ciphertext, uint8_t *plaintext) {
    ...
    for (int round = cipher->num_rounds - 1; round >= 0; round--) {
        if (cipher->algorithm_mode == MODE_FULL) {
            ...
            /* MODE 0: Minimal enhanced - reverse order */
            
            /* 4. Inverse cyclic bitwise rotation */
            int shift = (cipher->block_size > 1) ? cipher->round_keys[round][3] % 8 : round % 8;
            cyclic_bitwise_rotate(plaintext, cipher->block_size, shift, 1);
            
            /* 3. Inverse tertiary Rubik's cube rotation (Z-axis) */
            int op_id3 = cipher->round_keys[round][2] % NUM_OPERATIONS;
            int op_id1 = cipher->round_keys[round][0] % NUM_OPERATIONS;
            int op_id2 = cipher->round_keys[round][1] % NUM_OPERATIONS;
            if (op_id3 != op_id1 && op_id3 != op_id2) {
            ...
            /* 2. Inverse secondary Rubik's cube rotation (Y-axis) */
            if (op_id2 != op_id1) {
                apply_operation(cipher, plaintext, op_id2, 1);
            }
            
            /* 1. Inverse primary Rubik's cube rotation (X-axis) */
...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        printf("Режим / Mode: %s\n", algorithm_mode == MODE_FULL ? "1 (Full - 5 операций / operations)" : "0 (Minimal Enhanced - 4 операции: 3x Rubik + rotation / 4 operations: 3x Rubik + rotation)");
