/*
 * WBC1-CASCADE-NEW02 -- легкие операции + раундовая диффузия
 * ===========================================================
 ...
 *
 * Build:
 *   gcc -O3 -march=native -o wbc1_cascade_new02 wbc1_cascade_new02.c -lssl -lcrypto -lm
 */
...
#define MAGIC_LEN    4
#define VERSION      0x01
#define VERSION_CASC 0x06   /* NEW02: lightweight op table + round diffusion */
...
static void build_perm_table(int block_size) {
    /* Compatibility placeholder: operations are executed directly. */
...
static void apply_operation(Cube *cube, int op_id, int inverse) {
    ...
    int dim = cube->dim;
    int n   = dim * dim * dim;
    uint8_t *buf = cube->data;

    if (op <= 17) {
        int face = op / 3;
        int k = dir_map[op % 3];
        if (inverse && k != 2) k = -k;
        rotate_slice(cube, face_axis[face], face_pos[face], k);
        return;
    }

    if (op >= 18 && op <= 53) {
        int group = (op - 18) / 3;      /* 0..11 */
        int k = dir_map[(op - 18) % 3];
        if (inverse && k != 2) k = -k;

        int axis = group / 2;
        int layer_class = group % 2;
        int is_second = (op >= 36);
        int near = is_second ? 2 : 1;
        int far  = dim - 1 - near;
        if (near >= dim) near = dim - 1;
        if (far < 0) far = 0;
        int pos = layer_class ? far : near;
    ...
    if (op >= 63 && op <= 70) {
        int rot = (op - 63) + 1;
        for (int i = 0; i < n; i++) buf[i] = inverse ? rotate_right(buf[i], rot) : rotate_left(buf[i], rot);
        return;
    }

    if (op >= 71 && op <= 78) {
        int rot = (op - 71) + 1;
        for (int i = 0; i < n; i++) buf[i] = inverse ? rotate_left(buf[i], rot) : rotate_right(buf[i], rot);
        return;
    }

    if (op >= 79 && op <= 94) {
        int sh = (op - 79) + 1;
        rotate_bytes_block(buf, sh, inverse, n);
        return;
    }
    ...
    switch (op) {
        case 95:
            ...
            return;
        case 96:
            for (int i = 0; i < n / 2; i++) { uint8_t t = buf[i]; buf[i] = buf[n - 1 - i]; buf[n - 1 - i] = t; }
            return;
        case 97:
            for (int i = 0; i + 2 < n; i += 4) { uint8_t t = buf[i]; buf[i] = buf[i + 2]; buf[i + 2] = t; }
            return;
        case 98:
            for (int i = 1; i + 2 < n; i += 4) { uint8_t t = buf[i]; buf[i] = buf[i + 2]; buf[i + 2] = t; }
            return;
        case 99:
            rotate_bytes_block(buf, 1 + (n % 7), inverse, n);
            return;
        case 100:
            rotate_bytes_block(buf, 3 + (n % 11), inverse, n);
            return;
        case 101:
            rotate_bytes_block(buf, n / 2, inverse, n);
            return;
        case 102: {
            int q = n / 4;
            if (q > 0) {
        ...
        case 103: xor_mask_pattern(buf, n, 17, 0x5a); return;
        case 104: xor_mask_pattern(buf, n, 29, 0xa5); return;
        case 105: xor_mask_pattern(buf, n, 43, 0x3c); return;
        case 106: xor_mask_pattern(buf, n, 61, 0xc3); return;
        case 107:
            for (int i = 0; i + 1 < n; i += 2) { uint8_t t = buf[i]; buf[i] = buf[i + 1]; buf[i + 1] = t; }
            return;
        case 108:
        ...
        case 111:
            ...
            return;
        case 112:
            rotate_bytes_block(buf, n / 4, inverse, n);
            return;
        case 113:
            for (int i = 0; i < dim / 2; i++) swap_layers(cube, 1, i, dim - 1 - i);
            return;
        case 114:
            for (int i = 0; i < dim / 2; i++) swap_layers(cube, 2, i, dim - 1 - i);
            return;
        case 115:
            for (int i = 0; i < dim / 2; i++) swap_layers(cube, 0, i, dim - 1 - i);
            return;
        case 116:
            for (int i = 0; i + 1 < n; i += 2) {
                int p = i / 2;
                if ((p & 1) == 0) { uint8_t t = buf[i]; buf[i] = buf[i + 1]; buf[i + 1] = t; }
            }
            return;
        ...
        case 121: case 122: case 123: case 124: {
            int k = (op - 117) + 1;
            for (int i = 0; i < n; i++) {
                uint8_t m = (uint8_t)((i * 13) + (op * 7));
                if (!inverse) buf[i] = rotate_left((uint8_t)(buf[i] ^ m), k);
                else          buf[i] = (uint8_t)(rotate_right(buf[i], k) ^ m);
            }
            return;
        }
        case 125:
            for (int i = 0; i < n; i++) {
                uint8_t m = (uint8_t)(((n - 1 - i) * 9) + 0x5d);
                if (!inverse) buf[i] = rotate_left((uint8_t)(buf[i] ^ m), 3);
                else          buf[i] = (uint8_t)(rotate_right(buf[i], 3) ^ m);
            }
            return;
        case 126:
            for (int i = 0; i < n; i++) {
                uint8_t m = (uint8_t)((i * 5) ^ 0xa7);
                if (!inverse) buf[i] = rotate_left((uint8_t)(buf[i] ^ m), 2);
                else          buf[i] = (uint8_t)(rotate_right(buf[i], 2) ^ m);
            }
            return;
        default:
            return;
...
/* ── block cipher: operation -> XOR RK -> ROTL -> accum diffusion ──────── */

static void accum_diffuse_forward(uint8_t *buf, int n) {
    for (int i = 1; i < n; i++) buf[i] ^= buf[i - 1];
}

static void accum_undiffuse_forward(uint8_t *buf, int n) {
    for (int i = n - 1; i >= 1; i--) buf[i] ^= buf[i - 1];
}

static void accum_diffuse_backward(uint8_t *buf, int n) {
    for (int i = n - 2; i >= 0; i--) buf[i] ^= buf[i + 1];
}

static void accum_undiffuse_backward(uint8_t *buf, int n) {
...
static void encrypt_block(const uint8_t *key_mat, const uint8_t *block,
    ...
    /* 2. 32 rounds: op -> XOR round key -> ROTL -> accum diffusion */
    for (int r = 0; r < ROUNDS; r++) {
        ...
        for (int i = 0; i < N; i++)
            buf[i] = rotate_left(buf[i], alpha);

        if ((r & 1) == 0) accum_diffuse_forward(buf, N);
        else              accum_diffuse_backward(buf, N);
    }
...
static void decrypt_block(const uint8_t *key_mat, const uint8_t *block,
    ...
    for (int r = ROUNDS - 1; r >= 0; r--) {
        ...
        if ((r & 1) == 0) accum_undiffuse_forward(buf, N);
        else              accum_undiffuse_backward(buf, N);

        for (int i = 0; i < N; i++)
            buf[i] = rotate_right(buf[i], alpha);
...
int main(int argc, char *argv[]) {
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║   WBC1-CASCADE-NEW02 (32 раунда: лёгкие ops + диффузия) ║\n");
    printf("╠══════════════════════════════════════════════════════════╣\n");
    printf("║  Каскад:  %-46s ║\n",
