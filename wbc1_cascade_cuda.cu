...
/* ─── single-block round helpers (reused by single- and double-pass kernels) */
...
/* ── double-cascade encrypt: forward pass + backward pass ──────────────────
 * Pass 1 (→): blocks 0..n-1,   key chain starts from master_key
 * Pass 2 (←): blocks n-1..0,   key chain starts from derive(master_key)
 * Bidirectional avalanche: any 1-bit change diffuses across ALL blocks.    */
__global__ void encrypt_double_kernel(
    const uint8_t  *d_key,
    const uint8_t  *d_in,
    uint8_t        *d_out,
    ...
    const uint16_t *d_perms_g)
{
    ...
    /* ── pass 2: backward (derive rk_bwd from master_key) ── */
    if(tid < KEY_SIZE) s_ckey[tid] = d_key[msg_id * KEY_SIZE + tid];
    __syncthreads();
...
/* ── double-cascade decrypt: undo backward pass + undo forward pass ────────
 * Symmetric inverse of encrypt_double_kernel.                               */
__global__ void decrypt_double_kernel(
    const uint8_t  *d_key,
    const uint8_t  *d_in,
    uint8_t        *d_out,
    ...
    const uint16_t *d_perms_g)
{
    ...
    /* ── pass 1: undo backward encrypt (right-to-left) ── */
    if(tid < KEY_SIZE) s_ckey[tid] = d_key[msg_id * KEY_SIZE + tid];
    __syncthreads();
    d_derive_cascade_key(s_ckey, KEY_SIZE, s_ckey, tid);
    for(int blk = n_blocks - 1; blk >= 0; blk--){
        const uint8_t *enc = d_in  + in_off  + (size_t)blk * block_size;
        uint8_t       *dst = d_out + out_off + (size_t)blk * block_size;
        for(int i = tid; i < block_size; i += T) s_cube[i] = enc[i];
        __syncthreads();
        d_decrypt_block_rounds(s_cube, s_tmp, s_ckey, s_rk, block_size, tid, T, d_perms_g);
        for(int i = tid; i < block_size; i += T) dst[i] = s_cube[i];
        __syncthreads();
        /* derive from CIPHERTEXT enc[blk] (still valid in d_in) */
        d_derive_cascade_key(enc, block_size, s_ckey, tid);
    }

    /* ── pass 2: undo forward encrypt (left-to-right, in-place on d_out) ── */
    if(tid < KEY_SIZE) s_ckey[tid] = d_key[msg_id * KEY_SIZE + tid];
    __syncthreads();
    for(int blk = 0; blk < n_blocks; blk++){
        uint8_t *p = d_out + out_off + (size_t)blk * block_size;
        ...
        __syncthreads();
        d_decrypt_block_rounds(s_cube, s_tmp, s_ckey, s_rk, block_size, tid, T, d_perms_g);
        /* derive from intermediate ciphertext (p[]) BEFORE overwriting */
        if(blk < n_blocks - 1)
...
/* Encrypt a single message using GPU.
...
/* ── CPU double-cascade: forward + backward ──────────────────────────────── */
static uint8_t *cpu_cascade_encrypt_double(const uint8_t *key, const uint8_t *data,
                                            size_t data_len, int block_size, size_t *enc_len)
{
    build_perm_table_cpu(block_size);
    size_t padded_len;
    uint8_t *padded = wbc1_pad(data, data_len, block_size, &padded_len);
    ...
    free(padded);
    /* pass 2: backward (rk_bwd = derive(master_key, KEY_SIZE)) */
    uint8_t *out = (uint8_t*)malloc(padded_len);
    uint8_t rk_bwd[KEY_SIZE]; cpu_derive_cascade_key(key, KEY_SIZE, rk_bwd);
...
static uint8_t *cpu_cascade_decrypt_double(const uint8_t *key, const uint8_t *enc,
                                            size_t enc_len, int block_size, size_t *dec_len)
{
    build_perm_table_cpu(block_size);
    int n_blocks = (int)(enc_len / block_size);
    /* pass 1: undo backward (rk_bwd = derive(master_key, KEY_SIZE)) */
    ...
    for(int b = n_blocks-1; b >= 0; b--){
        ...
        cpu_derive_cascade_key(enc+(size_t)b*block_size, block_size, rk_bwd); /* from ciphertext */
    }
    /* pass 2: undo forward */
    uint8_t *out = (uint8_t*)malloc(enc_len);
    uint8_t ckey[KEY_SIZE]; memcpy(ckey, key, KEY_SIZE);
...
static int run_statistical_analysis(const uint8_t key[KEY_SIZE]){
    ...
    printf("\n  Statistical Analysis  (CPU encrypt, random key)\n");
    printf("  ─────────────────────────────────────────────────────────────\n");
    ...
    h_perm_bs = -1;
    size_t enc_len = 0;
    uint8_t *enc = cpu_cascade_encrypt_double(key, plain_stat, (size_t)STAT_SZ, bsz_stat, &enc_len);
...
static int self_test(const uint8_t key[KEY_SIZE]){
    printf("\n  Self-tests (double cascade, %s)\n", g_has_cuda ? "GPU" : "CPU fallback");
    int pass=0,fail=0;
    ...
    for(int si=0;si<ns;si++){
        ...
        uint8_t *enc = g_has_cuda
            ? cuda_cascade_encrypt_double(key,plain,(size_t)sz,bsz,&enc_len)
            : cpu_cascade_encrypt_double(key,plain,(size_t)sz,bsz,&enc_len);
        uint8_t *dec = g_has_cuda
            ? cuda_cascade_decrypt_double(key,enc,enc_len,bsz,&dec_len)
            : cpu_cascade_decrypt_double(key,enc,enc_len,bsz,&dec_len);
...
static int load_benchmark_sizes(int *sizes, int max_sizes){
    static const int defaults[]={1,10,100,1000,10000,100000,1000000,10000000,100000000};
...
static int query_device_concurrent_blocks(int cipher_block_size){
    ...
    if(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &enc_per_sm, encrypt_double_kernel, threads, shm) != cudaSuccess)
        return 1;
    if(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &dec_per_sm, decrypt_double_kernel, threads, shm) != cudaSuccess)
        return 1;
 ...
 *   (a) total cluster work = 256 MB (split evenly → time scales with GPU count)
...
static int auto_bench_batch_msgs(size_t padded_len, size_t msg_sz,
    ...
    /* (a) cluster-wide sizing: target ~256 MB total work. */
    const size_t target_total = 256u * 1024u * 1024u;
    ...
    /* (b) occupancy floor: for cascade-sequential kernels throughput/block is
     * constant, but we still want all SMs busy to measure peak GPU throughput.
     * For large messages this inflates total work above 256 MB (e.g. 840 MB
     * for 10 MB messages with 84 SMs) — the Batch/Work columns in the
     * benchmark table make this transparent.
     * Can be overridden with WBC_CUDA_MIN_BLOCKS. */
    {
        size_t min_blocks;
...
/* Dynamic block-size selection for GPU mode.
 * Goal: maximize useful parallel work per cascade step while accounting for
 * rank count and runtime batch limits (VRAM/RAM/occupancy floor).
 *
 * Notes:
 *  - For data_len <= MAX_BLOCK, minimal fitting cube size is kept (low padding).
 *  - For larger messages we evaluate larger cube sizes and pick the best score:
 *      score = batch_msgs / n_blocks_per_message
 *    where batch_msgs already includes world_size and hardware limits.
 */
static int auto_block_size_dynamic(size_t data_len, int world_size){
    const char *env_bsz = getenv("WBC_CUDA_BLOCK_SIZE");
    if(env_bsz){
        int v = atoi(env_bsz);
        for(int i = 0; i < NUM_CUBE_SIZES; i++)
            if(CUBE_SIZES[i].n == v) return v;
    }

    int base = auto_block_size((data_len > 2147483647ULL) ? 2147483647 : (int)data_len);
    if(!g_has_cuda) return base;
    if(data_len <= (size_t)MAX_BLOCK) return base;

    int best_bsz = base;
    double best_score = -1.0;
    size_t best_padded = (size_t)-1;

    for(int i = 0; i < NUM_CUBE_SIZES; i++){
        int bsz = CUBE_SIZES[i].n;
        if(bsz < 1000) continue; /* tiny cubes are never efficient for large text */

        size_t padded_len = wbc1_padded_len(data_len, bsz);
        int n_blocks = (int)(padded_len / (size_t)bsz);
        if(n_blocks < 1) n_blocks = 1;

        int batch_msgs = auto_bench_batch_msgs(padded_len, data_len, world_size, bsz);
        double score = (double)batch_msgs / (double)n_blocks;

        if(score > best_score ||
           (fabs(score - best_score) < 1e-12 && padded_len < best_padded)){
            best_score = score;
            best_bsz = bsz;
            best_padded = padded_len;
        }
    }
...
static void benchmark(const uint8_t key[KEY_SIZE]){
    int world_rank = 0;
    int world_size = 1;
    ...
    double gpu_clock_hz = 0.0;
    {
        ...
        if(cudaGetDevice(&dev) == cudaSuccess &&
           ...
           prop.clockRate > 0)
            gpu_clock_hz = (double)prop.clockRate * 1000.0; /* kHz -> Hz */
    }

    build_op_table(key);

    if(world_rank == 0){
        printf("\nPerformance Benchmark (GPU, double-cascade):\n");
        printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        printf("  %10s  %10s  %14s  %14s  %12s  %12s  %14s  %14s  %10s  %10s  %s\n",
            "Size (KB)", "Enc (s)", "Enc (KB/s)", "Dec (KB/s)",
            "Enc (MB/s)", "Dec (MB/s)", "Enc (Mbit/s)", "Dec (Mbit/s)", "Enc CPB", "Dec CPB", "Integrity");
        printf("  %s\n","-------------------------------------------------------------------------------------------------------------------------------------------");
        printf("  Timing mode: MPI_MAX across ranks (time decreases with more GPUs)\n");
        printf("  Workload: 256 MB total split equally across %d ranks (~%d MB/rank)\n",
               world_size, (int)(256 / world_size));
        printf("  Throughput: aggregate = total_msgs_all_ranks * msg_size / max_rank_time\n");
        printf("  Scaling mode: fixed ~256 MB/rank (set WBC_CUDA_MIN_BLOCKS=N to force N-block floor)\n");
        printf("  CPB: cycles/byte using current rank-0 GPU clock\n");
    ...
    for(int si=0;si<ns;si++){
        int sz=sizes[si];
        int bsz=auto_block_size_dynamic((size_t)sz, world_size);
        ...
        if(world_rank == 0){
            ...
            double total_bytes = (double)sz * (double)batch_msgs * (double)world_size;
            ...
            double enc_cpb = (gpu_clock_hz > 0.0 && total_bytes > 0.0)
                ? (ge * gpu_clock_hz) / total_bytes : 0.0;
            double dec_cpb = (gpu_clock_hz > 0.0 && total_bytes > 0.0)
                ? (gd * gpu_clock_hz) / total_bytes : 0.0;

            printf("  %10.2f  %10.5f  %14.2f  %14.2f  %12.4f  %12.4f  %14.4f  %14.4f  %10.4f  %10.4f  %s\n",
...
static int run_encrypt_task(const uint8_t key[KEY_SIZE], int mode, int data_kb, const char *custom_text){
    uint8_t *plain = NULL;
    size_t plain_len = 0;
    ...
    int world_size = 1;
    MPI_Comm_size(MPI_COMM_WORLD, &world_size);
    int block_size = auto_block_size_dynamic(plain_len, world_size);
    build_op_table(key);
    ...
    clock_gettime(CLOCK_MONOTONIC,&t0);
    uint8_t *enc = g_has_cuda
        ? cuda_cascade_encrypt_double(key, plain, plain_len, block_size, &enc_len)
        : cpu_cascade_encrypt_double(key, plain, plain_len, block_size, &enc_len);
    clock_gettime(CLOCK_MONOTONIC,&t1);
    double enc_t = (t1.tv_sec-t0.tv_sec)+(t1.tv_nsec-t0.tv_nsec)/1e9;

    clock_gettime(CLOCK_MONOTONIC,&t0);
    uint8_t *dec = g_has_cuda
        ? cuda_cascade_decrypt_double(key, enc, enc_len, block_size, &dec_len)
        : cpu_cascade_decrypt_double(key, enc, enc_len, block_size, &dec_len);
    clock_gettime(CLOCK_MONOTONIC,&t1);
    ...
    if(use_named_cli){
        ...
        for(int i=1;i<argc;i++){
            ...
            if(strcmp(argv[i], "--task-encrypt") == 0 || strcmp(argv[i], "--encrypt") == 0){
                ...
                if(world_rank == 0){
                    printf("Usage (named): %s --task <encrypt|analysis|benchmark> [--mode demo|random] [--size KB] [--text STRING]\n", argv[0]);
        ...
        if(task == 1){
            build_op_table(key);
            int rc = self_test(key);
            if(world_rank == 0)
                run_statistical_analysis(key);
            MPI_Finalize();
            return rc;
        }
        int rc = run_encrypt_task(key, mode, data_kb, custom_text);
        MPI_Finalize();
        return rc;
    }
