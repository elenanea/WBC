 ...
 */
/*
 * cascade_encrypt: same as wbc1_encrypt but key evolves after every block.
 * Key update source:
...
static uint8_t *cascade_encrypt(const uint8_t master_key[KEY_SIZE],
    ...
    /* inter = output of pass 1 (mode-encrypted);  result = output of pass 2 (backward) */
    ...
    /* ── Pass 1: forward mode-based cascade ─────────────────────────────── */
    uint8_t rk[KEY_SIZE];
    ...
    for (size_t b = 0; b < n_blocks; b++) {
        ...
        uint8_t *out_blk   = inter + b * block_bytes;
    ...
    /* ── Pass 2: backward ECB cascade ───────────────────────────────────── */
    /* Independent backward key: derived from master_key itself. */
...
static uint8_t *cascade_decrypt(const uint8_t master_key[KEY_SIZE],
    ...
    /* ── Undo Pass 1: forward mode-based cascade ─────────────────────────── */
    uint8_t rk[KEY_SIZE];
    memcpy(rk, master_key, KEY_SIZE);
...
/* Per-block Strict Avalanche Criterion: для кожного блоку перебираємо всі
 * fixed_bs*8 бітів — шифруємо з кожним перевернутим бітом і усереднюємо. */
...
static void avalanche_test(const uint8_t key[KEY_SIZE], const uint8_t *data,
    ...
    /* Stream modes (CTR/CFB/OFB/WBC-CTR-HMAC) are XOR-based: flipping 1 pt
     * bit flips exactly 1 ct bit by design.  A plaintext-avalanche test
     * always gives ~0% for them and is meaningless.  Instead we test KEY
     * avalanche: how many ct bits change when 1 key bit is flipped. */
    ...
    /* Number of items to flip and pointer to the flip source */
    ...
    for (size_t i = 0; i < flip_count; i++) {
        ...
        if (is_stream) {
            ...
            /* flip plaintext bit i */
            uint8_t *mod = (uint8_t *)malloc(data_len);
            memcpy(mod, data, data_len);
            mod[i / 8] ^= (uint8_t)(1 << (i % 8));

            if (mode == MODE_WBC_CTR_HMAC) {
                enc1 = cascade_encrypt_ctr_hmac(key, mod, data_len, block_size, nonce0, &enc_len1);
                size_t hdr = MAGIC_LEN + 1 + 2 + NONCE_SIZE;
                const uint8_t *cmp = enc1 + hdr;
                size_t cmp_len = enc_len1 - hdr - MAC_SIZE;
                if (cmp_len == ref_len)
                ...
                uint8_t *iv1 = NULL; size_t iv1_len = 0;
                enc1 = cascade_encrypt(key, mod, data_len, mode, iv0, block_size,
                                    &iv1, &iv1_len, &enc_len1);
                if (enc_len1 == ref_len)
                    for (size_t j = 0; j < enc_len1; j++)
                        total_flips += __builtin_popcount(enc0[j] ^ enc1[j]);
    ...
    if (is_stream)
        printf("  Avalanche effect (key):  %.2f%%  (ideal ~50%%, stream mode — key-bit flip)\n",
               ratio * 100.0);
    else
