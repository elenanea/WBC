...
static void derive_cascade_key(const uint8_t *enc_block, int block_bytes,
     ...
     * Тест показал: лавина ~50% уже при r=1; 4 — консервативный запас. */
    for (int r = 0; r < 4; r++) {
    ...
    /* Шаг 3: берём первые KEY_SIZE байт (без compress, чтобы не терять энтропию) */
...
static void derive_parallel_key(const uint8_t master[KEY_SIZE], size_t block_idx,
    ...
    c = F[0];
    for (int i = KEY_SIZE - 1; i >= 0; i--) { c = F[i] ^ rotate_right(c, 3); out[i] = c; }
}
...
static void benchmark(const uint8_t key[32], EncMode mode, int double_pass) {
    ...
    static const int sizes_default[] = {1,10,100,1000,10000,100000,1000000,10000000};
    static const int sizes_with_1gb[] = {1,10,100,1000,10000,100000,1000000,10000000,1073741824};
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        for (int i=1; i<argc; i++) {
            if (strcmp(argv[i],"-s")==0 || strcmp(argv[i],"--single")==0) double_pass=0;
                ...
                if (cli_data_size <= 0 || cli_data_size > 1000000) cli_data_size = 1024;
                ...
                printf("Usage: mpirun -n N %s [--single|--double] [--task TASK] [--mode MODE] [--text TXT] [--size N] [--once] [--bench-1gb]\n", argv[0]);
    ...
    if (rank == 0) {
        printf("\n");
        printf("╔══════════════════════════════════════════════════════════╗\n");
        printf("║        PWBC1.1 — WBC1-CASCADE-NEW  (MPI parallel)       ║\n");
        printf("╠══════════════════════════════════════════════════════════╣\n");
    ...
    while (1) {
        ...
        if (task == TASK_SWITCH_MODE) {
            if (rank==0) double_pass = !double_pass;
            MPI_Bcast(&double_pass, 1, MPI_INT, 0, MPI_COMM_WORLD);
            if (rank==0)
            ...
            if (rank == 0) {
                ...
                if (cli_task != 0) {
                    if (cli_text[0] != 0) {
                        ...
                        strncpy(text, "Hello from WBC1 CLI mode", sizeof(text)-1);
                    }
                } else {
