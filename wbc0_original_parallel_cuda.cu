...
typedef struct {
    ...
    int subblock_size_override_bytes;
...
static unsigned int g_shift_map_cache_cursor = 0;
...
typedef struct {
    ...
    int bitmap_shift_bits;
} CudaShiftBuffers;

static CudaShiftBuffers g_cuda_shift_buffers = {0};

static int cuda_is_available(void) {
    if (!g_cuda_checked) {
        int device_count = 0;
        cudaError_t err = cudaGetDeviceCount(&device_count);
        g_cuda_available = (err == cudaSuccess && device_count > 0) ? 1 : 0;
        g_cuda_checked = 1;
    }
    return g_cuda_available;
}

static void print_cuda_runtime_status_once(int rank) {
    if (rank != 0) {
        return;
    }
    ...
    if (err == cudaSuccess && device_count > 0) {
        printf("CUDA runtime: available (%d device(s)). GPU kernel path enabled.\n", device_count);
    } else {
        printf("CUDA runtime: unavailable. Using CPU fallback path.\n");
    }
}

static cudaError_t ensure_cuda_shift_buffers(void) {
    if (g_cuda_shift_buffers.initialized) {
        return cudaSuccess;
    }

    size_t max_block_len = (size_t)MAX_BLOCK_SIZE;
    size_t max_bits_len = (size_t)(MAX_BLOCK_SIZE * 8);

    cudaError_t ok = cudaSuccess;
    ok = cudaMalloc((void **)&g_cuda_shift_buffers.d_temp, max_block_len);
    if (ok == cudaSuccess) ok = cudaMalloc((void **)&g_cuda_shift_buffers.d_block, max_block_len);
    if (ok == cudaSuccess) ok = cudaMalloc((void **)&g_cuda_shift_buffers.d_src_byte, max_bits_len * sizeof(uint8_t));
    if (ok == cudaSuccess) ok = cudaMalloc((void **)&g_cuda_shift_buffers.d_src_mask, max_bits_len * sizeof(uint8_t));
    if (ok == cudaSuccess) ok = cudaMalloc((void **)&g_cuda_shift_buffers.d_dst_byte, max_bits_len * sizeof(uint8_t));
    if (ok == cudaSuccess) ok = cudaMalloc((void **)&g_cuda_shift_buffers.d_dst_mask, max_bits_len * sizeof(uint8_t));

    if (ok != cudaSuccess) {
        return ok;
    }

    g_cuda_shift_buffers.initialized = 1;
    g_cuda_shift_buffers.bitmap_size_bytes = -1;
    g_cuda_shift_buffers.bitmap_shift_bits = -1;
    return cudaSuccess;
}

static void release_cuda_shift_buffers(void) {
    if (g_cuda_shift_buffers.d_temp) cudaFree(g_cuda_shift_buffers.d_temp);
    if (g_cuda_shift_buffers.d_block) cudaFree(g_cuda_shift_buffers.d_block);
    if (g_cuda_shift_buffers.d_src_byte) cudaFree(g_cuda_shift_buffers.d_src_byte);
    if (g_cuda_shift_buffers.d_src_mask) cudaFree(g_cuda_shift_buffers.d_src_mask);
    if (g_cuda_shift_buffers.d_dst_byte) cudaFree(g_cuda_shift_buffers.d_dst_byte);
    if (g_cuda_shift_buffers.d_dst_mask) cudaFree(g_cuda_shift_buffers.d_dst_mask);

    memset(&g_cuda_shift_buffers, 0, sizeof(g_cuda_shift_buffers));
}

static void bind_cuda_device_for_rank(int rank, int size) {
    int device_count = 0;
    cudaError_t err = cudaGetDeviceCount(&device_count);
    if (err != cudaSuccess || device_count <= 0) {
        return;
    ...
    if (set_err != cudaSuccess) {
        fprintf(stderr, "Rank %d: cudaSetDevice(%d) failed: %s\n", rank, device_id, cudaGetErrorString(set_err));
        return;
    }

    if (rank == 0 && size > device_count) {
        printf("Warning: MPI processes (%d) exceed CUDA devices (%d). Multiple ranks will share GPUs.\n", size, device_count);
    }

    printf("Rank %d bound to CUDA device %d/%d\n", rank, device_id, device_count);
}

__global__ static void cyclic_bitwise_shift_kernel(const uint8_t *temp,
                                                   uint8_t *block,
    ...
    if (temp[src_byte[idx]] & src_mask[idx]) {
        atomicOr((unsigned int *)&block[dst_byte[idx]], (unsigned int)dst_mask[idx]);
    }
}
...
static const ShiftBitMap *get_shift_bitmap(int size_bytes, int shift_bits, int total_bits) {
    for (int i = 0; i < SHIFT_MAP_CACHE_SIZE; i++) {
        ShiftBitMap *slot = &g_shift_map_cache[i];
        if (slot->valid && slot->size_bytes == size_bytes && slot->shift_bits == shift_bits) {
            return slot;
        }
    }

    ShiftBitMap *slot = &g_shift_map_cache[g_shift_map_cache_cursor % SHIFT_MAP_CACHE_SIZE];
    g_shift_map_cache_cursor++;

    slot->valid = 1;
    slot->size_bytes = size_bytes;
    slot->shift_bits = shift_bits;
    slot->total_bits = total_bits;
    ...
    for (int i = 0; i < total_bits; i++) {
        ...
        slot->src_byte[i] = (uint8_t)src_byte;
        slot->src_mask[i] = (uint8_t)(1u << src_pos);
        slot->dst_byte[i] = (uint8_t)dst_byte;
        slot->dst_mask[i] = (uint8_t)(1u << dst_pos);
    }
...
static void cyclic_bitwise_shift(uint8_t *block, int size_bytes, int shift_bits) {
    ...
    /* Create temporary buffer (stack for common sizes, heap fallback) */
    uint8_t local_temp[MAX_BLOCK_SIZE];
    uint8_t *temp = local_temp;
    int use_heap = 0;
    if (size_bytes > MAX_BLOCK_SIZE) {
        temp = (uint8_t *)malloc(size_bytes);
        if (!temp) {
            return;
        }
        use_heap = 1;
    ...
    const ShiftBitMap *bitmap = get_shift_bitmap(size_bytes, shift_bits, total_bits);
    size_t block_len = (size_t)size_bytes;

    if (cuda_is_available() && size_bytes <= MAX_BLOCK_SIZE) {
        cudaError_t ok = ensure_cuda_shift_buffers();

        if (ok == cudaSuccess) {
            ok = cudaMemcpy(g_cuda_shift_buffers.d_temp, temp, block_len, cudaMemcpyHostToDevice);
            if (ok == cudaSuccess) ok = cudaMemset(g_cuda_shift_buffers.d_block, 0, block_len);

            if (ok == cudaSuccess &&
                (g_cuda_shift_buffers.bitmap_size_bytes != size_bytes ||
                 g_cuda_shift_buffers.bitmap_shift_bits != shift_bits)) {
                ok = cudaMemcpy(g_cuda_shift_buffers.d_src_byte, bitmap->src_byte, total_bits * sizeof(uint8_t), cudaMemcpyHostToDevice);
                if (ok == cudaSuccess) ok = cudaMemcpy(g_cuda_shift_buffers.d_src_mask, bitmap->src_mask, total_bits * sizeof(uint8_t), cudaMemcpyHostToDevice);
                if (ok == cudaSuccess) ok = cudaMemcpy(g_cuda_shift_buffers.d_dst_byte, bitmap->dst_byte, total_bits * sizeof(uint8_t), cudaMemcpyHostToDevice);
                if (ok == cudaSuccess) ok = cudaMemcpy(g_cuda_shift_buffers.d_dst_mask, bitmap->dst_mask, total_bits * sizeof(uint8_t), cudaMemcpyHostToDevice);
        ...
        if (ok == cudaSuccess) {
            int threads = 256;
            int blocks = (total_bits + threads - 1) / threads;
            cyclic_bitwise_shift_kernel<<<blocks, threads>>>(g_cuda_shift_buffers.d_temp,
                                                             g_cuda_shift_buffers.d_block,
                                                             g_cuda_shift_buffers.d_src_byte,
                                                             g_cuda_shift_buffers.d_src_mask,
                                                             g_cuda_shift_buffers.d_dst_byte,
                                                             g_cuda_shift_buffers.d_dst_mask,
                                                             total_bits);
            ok = cudaGetLastError();
            if (ok == cudaSuccess) {
            ...
            if (ok == cudaSuccess) {
                ok = cudaMemcpy(block, g_cuda_shift_buffers.d_block, block_len, cudaMemcpyDeviceToHost);
            }
        }
    ...
    for (int i = 0; i < total_bits; i++) {
        if (temp[bitmap->src_byte[i]] & bitmap->src_mask[i]) {
            block[bitmap->dst_byte[i]] |= bitmap->dst_mask[i];
        }
    }
...
static void init_operations(WBC1OriginalCipher *cipher, const uint8_t *key, int key_len) {
    ...
    if (g_init_ops_cache.valid &&
        g_init_ops_cache.key_len == key_len &&
        memcmp(g_init_ops_cache.key, key, key_len) == 0) {
    ...
    if (key_len <= MAX_KEY_SIZE_BYTES) {
        ...
        g_init_ops_cache.key_len = key_len;
        memcpy(g_init_ops_cache.key, key, key_len);
...
static uint8_t rotl8(uint8_t value, int shift) {
    shift &= 7;
    return (uint8_t)((value << shift) | (value >> (8 - shift)));
}

static uint8_t rotr8(uint8_t value, int shift) {
    shift &= 7;
    return (uint8_t)((value >> shift) | (value << (8 - shift)));
}
...
static uint32_t hash_operation_seed(const Operation *op, int op_id, int size) {
    ...
    for (int f = 0; f < 3; f++) {
        const unsigned char *ptr = fields[f];
        while (*ptr) {
            hash ^= (uint32_t)(*ptr++);
            hash *= 16777619u;
        }
...
static int parse_turn_value(const char *turn) {
    if (!turn || turn[0] == '\0') return 1;
    if (strcmp(turn, "'") == 0) return -1;
    if (strcmp(turn, "2") == 0) return 2;
    if (strcmp(turn, "3") == 0) return 3;
    return 1;
}
...
static inline uint32_t op_param_cache_index(int op_id, int size) {
    uint32_t h = 2166136261u;
    h ^= (uint32_t)op_id;
    h *= 16777619u;
    h ^= (uint32_t)size;
    h *= 16777619u;
    return h % OP_PARAM_CACHE_SIZE;
}

static const CachedOpParams *get_cached_op_params(const Operation *op, int op_id, int size) {
    uint32_t idx = op_param_cache_index(op_id, size);
    CachedOpParams *slot = &g_op_param_cache[idx];

    if (slot->valid && slot->op_id == op_id && slot->size == size) {
        return slot;
    }
    ...
    if (strcmp(op->type, "swap") == 0) {
        slot->op_kind = 1;
        return slot;
    }

    if (strcmp(op->type, "diagflip") == 0) {
        slot->op_kind = 2;
        return slot;
    }
    ...
    int turn = parse_turn_value(op->param2);
    int base = (int)((seed >> 16) % (uint32_t)size) + 1;
    int shift = base * (turn < 0 ? -turn : turn);
    if (turn < 0) {
        shift = -shift;
    }
    slot->shift = shift;
...
static void apply_diagflip_style(uint8_t *block, int size) {
    ...
    for (int i = 0; i < size / 2; i++) {
        uint8_t t = block[i];
        block[i] = block[size - 1 - i];
        block[size - 1 - i] = t;
    }
}

static void apply_base_operation_permutation(uint8_t *block, int size, const Operation *op, int op_id, int inverse) {
    if (size <= 1 || !op) {
        return;
    }

    const CachedOpParams *params = get_cached_op_params(op, op_id, size);
    if (params->op_kind == 1) {
        apply_swap_style(block, size, params->seed);
        return;
    }

    if (params->op_kind == 2) {
        apply_diagflip_style(block, size);
        return;
    }

    if (!inverse) {
        rotate_block_bytes(block, size, params->shift);
        apply_affine_permutation(block, size, params->a, params->b, 0);
    } else {
        uint8_t temp[MAX_BLOCK_SIZE];
        memcpy(temp, block, size);
        for (int dst = 0; dst < size; dst++) {
            int src = (params->inv_a * (dst - params->b + size)) % size;
            block[src] = temp[dst];
        }
        rotate_block_bytes(block, size, -params->shift);
    }
...
static void apply_operation_sized(WBC1OriginalCipher *cipher, uint8_t *block, int size, int op_id, int inverse) {
    if (size <= 1) {
        return;
    }

    if (!cipher || !cipher->operations || !cipher->base_operations || op_id < 0 || op_id >= NUM_OPERATIONS) {
        return;
    }
    ...
    if (op->chain_length > 0) {
        if (!inverse) {
            for (int i = 0; i < op->chain_length; i++) {
                int base_idx = op->chain[i];
                if (base_idx < 0 || base_idx >= cipher->base_ops_count) {
                    continue;
                }
                Operation *base_op = &cipher->base_operations[base_idx];
                apply_base_operation_permutation(block, size, base_op, op_id * 257 + base_idx, 0);
            }
        } else {
            for (int i = op->chain_length - 1; i >= 0; i--) {
                ...
                Operation *base_op = &cipher->base_operations[base_idx];
                apply_base_operation_permutation(block, size, base_op, op_id * 257 + base_idx, 1);
            }
        }
        return;
    }

    apply_base_operation_permutation(block, size, op, op_id, inverse);
}
...
static uint8_t expand_round_key_byte(const uint8_t *round_key, int round_key_len, int index) {
    if (round_key_len <= 0) {
        return 0;
    }

    uint8_t center = round_key[index % round_key_len];
    uint8_t left = round_key[(index - 1 + round_key_len) % round_key_len];
    uint8_t right = round_key[(index + 1) % round_key_len];

    uint8_t mixed = center ^ rotl8(right, index & 7) ^ rotr8(left, (index + 3) & 7) ^ (uint8_t)(index * 29);
...
static void build_round_schedule(WBC1OriginalCipher *cipher,
                                 ...
                                 int *op_ids,
                                 int *shifts) {
    for (int byte_idx = 0; byte_idx < round_key_len; byte_idx++) {
        uint8_t mixed_byte = expand_round_key_byte(round_key, round_key_len, byte_idx);
        int key_bit = (mixed_byte >> 7) & 1;
        op_ids[byte_idx] = mixed_byte % NUM_OPERATIONS;
        shifts[byte_idx] = key_bit ? cipher->cube_d : (cipher->cube_d / 2 + 1);
    }
}
...
static void process_subblock_with_key(WBC1OriginalCipher *cipher,
                                      ...
                                      int round_key_len,
                                      int inverse) {
    memcpy(output, input, subblock_size);

    if (round_key_len <= 0) {
        return;
    }

    int op_ids[MAX_BLOCK_SIZE];
    int shifts[MAX_BLOCK_SIZE];
    if (round_key_len > MAX_BLOCK_SIZE) {
        return;
    }
    build_round_schedule(cipher, round_key, round_key_len, op_ids, shifts);

    if (!inverse) {
        for (int byte_idx = 0; byte_idx < round_key_len; byte_idx++) {
            int op_id = op_ids[byte_idx];

            apply_operation_sized(cipher, output, subblock_size, op_id, 0);

            int shift = shifts[byte_idx];
            cyclic_bitwise_shift(output, subblock_size, shift);
        }
    } else {
        for (int byte_idx = round_key_len - 1; byte_idx >= 0; byte_idx--) {
            int shift = shifts[byte_idx];
            cyclic_bitwise_shift(output, subblock_size, -shift);

            int op_id = op_ids[byte_idx];
            apply_operation_sized(cipher, output, subblock_size, op_id, 1);
        }
    }
}

static int get_subblock_size(WBC1OriginalCipher *cipher) {
    int subblock_size = cipher->cube_d;
    if (cipher->subblock_size_override_bytes > 0) {
        subblock_size = cipher->subblock_size_override_bytes;
    }
    if (subblock_size < 1) {
        subblock_size = 1;
    }
    if (subblock_size > cipher->block_size_bytes) {
        subblock_size = cipher->block_size_bytes;
    }
    return subblock_size;
}

static void derive_round_key_from_subblock_bits(const uint8_t *encrypted_subblock,
                                                int subblock_size_bytes,
                                                uint8_t *round_key,
                                                int round_key_len_bytes) {
    int total_subblock_bits = subblock_size_bytes * 8;
    int required_bits = round_key_len_bytes * 8;

    memset(round_key, 0, round_key_len_bytes);

    if (total_subblock_bits <= 0 || required_bits <= 0) {
        return;
    }

    for (int bit_idx = 0; bit_idx < required_bits; bit_idx++) {
        int src_bit_idx = bit_idx % total_subblock_bits;
        int src_byte = src_bit_idx / 8;
        int src_pos = 7 - (src_bit_idx % 8);
        int dst_byte = bit_idx / 8;
        int dst_pos = 7 - (bit_idx % 8);

        int bit_val = (encrypted_subblock[src_byte] >> src_pos) & 1;
        if (bit_val) {
            round_key[dst_byte] |= (1 << dst_pos);
        }
    }
}
...
void wbc1_original_encrypt_block(WBC1OriginalCipher *cipher, const uint8_t *plaintext, uint8_t *ciphertext) {
    ...
    int subblock_size = get_subblock_size(cipher);
    uint8_t current_key[MAX_BLOCK_SIZE];
    int current_key_len = cipher->key_len_bytes;

    memcpy(current_key, cipher->key, current_key_len);

    for (int offset = 0; offset < block_size; offset += subblock_size) {
        int cur_subblock_size = min_int(subblock_size, block_size - offset);

        process_subblock_with_key(cipher,
                                  plaintext + offset,
                                  ciphertext + offset,
                                  cur_subblock_size,
                                  current_key,
        ...
        current_key_len = cipher->key_len_bytes;
        derive_round_key_from_subblock_bits(ciphertext + offset,
                                            cur_subblock_size,
                                            current_key,
                                            current_key_len);
    }
...
static void print_key_operation_mapping(WBC1OriginalCipher *cipher, int show_count) {
    ...
    for (int i = 0; i < show_count; i++) {
        ...
        printf("Key[%3d]: %c 0x%02X (mixed=0x%02X) → Op %3d: ", i, ascii_char, key_byte, mixed_byte, op_id);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        if (rank == 0) {
            printf("Usage: %s <task> <key_size> <key_source> <block_size_bits> [mode] [data_size_kb] [seed] [avalanche_tests] [subblock_size_bytes]\n", argv[0]);
            ...
            printf("  seed: fixed RNG seed for reproducible tests (optional)\n");
            printf("  avalanche_tests: number of avalanche iterations in task 1 (optional, default 300)\n");
            printf("  subblock_size_bytes: manual subblock size in bytes (optional, 1..block_size_bytes)\n");
            printf("\nExamples:\n");
            ...
            printf("  %s 1 256 0 128 1 100 42 500  # Tests with fixed seed and 500 avalanche runs\n", argv[0]);
            printf("  %s 1 256 0 512 1 1000 42 500 4  # Force 4-byte subblocks\n", argv[0]);
    ...
    print_cuda_runtime_status_once(rank);
    bind_cuda_device_for_rank(rank, size);
    ...
    int mode = 0;
    int avalanche_runs = 300;
    int subblock_override_bytes = 0;
    ...
    if (argc >= 8) {
        rng_seed = (unsigned int)strtoul(argv[7], NULL, 10);
        use_fixed_seed = 1;
    }
    if (argc >= 9) {
        avalanche_runs = atoi(argv[8]);
        if (avalanche_runs < 10) avalanche_runs = 10;
        if (avalanche_runs > 5000) avalanche_runs = 5000;
    }
    if (argc >= 10) {
        subblock_override_bytes = atoi(argv[9]);
        if (subblock_override_bytes < 1) subblock_override_bytes = 1;
    }

    /* Seed RNG once per process */
    if (use_fixed_seed) {
        srand(rng_seed ^ (unsigned int)(rank * 0x9e3779b9u));
    } else {
        srand((unsigned int)time(NULL) ^ (unsigned int)(rank * 0x9e3779b9u));
    }
    ...
    if (subblock_override_bytes > 0) {
        int max_subblock = cipher.block_size_bytes;
        if (subblock_override_bytes > max_subblock) {
            subblock_override_bytes = max_subblock;
        }
        cipher.subblock_size_override_bytes = subblock_override_bytes;
    }
    int subblock_size_bytes = get_subblock_size(&cipher);
    int subblock_size_bits = subblock_size_bytes * 8;
    int subblocks_per_block = (cipher.block_size_bytes + subblock_size_bytes - 1) / subblock_size_bytes;
    ...
    if (task == 0) {
        ...
        if (rank == 0) {
            ...
            printf("Subblock size: %d bits (%d bytes)\n", subblock_size_bits, subblock_size_bytes);
            printf("Subblocks per block: %d\n", subblocks_per_block);
        ...
        /* Ensure task-1 uses the actual requested size */
        ...
        if (rank == 0) {
            ...
            printf("Subblock size / Размер подблока: %d bits\n", subblock_size_bits);
            printf("Subblocks per block / Кол-во подблоков на блок: %d\n", subblocks_per_block);
            printf("Key size / Размер ключа: %d bits\n", key_size);
            printf("Avalanche runs / Кол-во прогонов лавины: %d\n", avalanche_runs);
            if (use_fixed_seed) {
                printf("RNG seed / Сид ГПСЧ: %u\n", rng_seed);
            }
            printf("\n");
            ...
            avalanche_test(&cipher, avalanche_runs, avalanche_results);
