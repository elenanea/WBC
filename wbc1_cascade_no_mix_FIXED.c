/*
 * WBC1-Cascade -- C implementation
 * ==================================
 * Identical to wbc1_fixed.c EXCEPT the key evolves after each block:
 *
 *   RK^(0) = master_key (256-bit)
 *   B = encrypt_block(RK^(r), ...)     <- unchanged block cipher
 *   z_i   = B[i%bb]
 *           ^ ROTL8(B[(i+1)%bb],    i%8)
 *           ^ ROTR8(B[(i-1+bb)%bb], (i+3)%8)
 *           ^ (29*i % 256)
 *   m_i   = z_i ^ (z_i>>4) ^ ROTL8(z_i,3) ^ (z_i>>1)
 *   RK^(r+1)[i] = m_i
 *
 * Build:
 *   gcc -O2 -o wbc1_fixed_cascade wbc1_fixed_cascade.c -lssl -lcrypto -lm
 */
 ...
 * ==========================================================================
 * Step 1 — XOR-fold enc_block into KEY_SIZE bytes so that EVERY byte of
 *           enc_block (regardless of block_bytes) influences the result:
 *
 *   F[j] = XOR of enc_block[k] for all k where k % KEY_SIZE == j
 *
 * Step 2 — apply mixing formula on the folded KEY_SIZE-byte array F:
 *
 *   z_i   = F[i]
 *           ^ ROTL8(F[(i+1) % KEY_SIZE],       i % 8)
 *           ^ ROTR8(F[(i-1+KEY_SIZE)%KEY_SIZE], (i+3) % 8)
 *           ^ (29*i % 256)
 *   new_rk[i] = z_i ^ (z_i>>4) ^ ROTL8(z_i,3) ^ (z_i>>1)
 */
static void derive_cascade_key(const uint8_t *enc_block, int block_bytes,
                                uint8_t new_rk[KEY_SIZE]) {
    /* Step 1: fold all block bytes into KEY_SIZE slots */
    uint8_t F[KEY_SIZE] = {0};
    for (int j = 0; j < block_bytes; j++)
        F[j % KEY_SIZE] ^= enc_block[j];

    /* Step 2: mixing formula */
    for (int i = 0; i < KEY_SIZE; i++) {
        uint8_t zi = F[i]
            ^ rotate_left (F[(i + 1) % KEY_SIZE],              i % 8)
            ^ rotate_right(F[(i - 1 + KEY_SIZE) % KEY_SIZE], (i + 3) % 8)
            ^ (uint8_t)((29 * i) % 256);
        new_rk[i] = (uint8_t)(zi ^ (zi >> 4)
                              ^ (uint8_t)((zi << 3) | (zi >> 5))
                              ^ (zi >> 1));
    }
}
 ...
 */
/*
 * cascade_encrypt: single forward cascade — key evolves after every block.
 * Key update source:
 *   ECB/CBC  -> output of encrypt_block (= ciphertext block)
 *   CFB/OFB/CTR -> output of encrypt_block before XOR (= keystream block)
 * NOTE: no backward pass (WBC1 — single cascade only).
 */
static uint8_t *cascade_encrypt(const uint8_t master_key[KEY_SIZE],
    ...
    for (size_t b = 0; b < n_blocks; b++) {
        ...
        if (mode == MODE_ECB) {
            encrypt_block(rk, blk, block_size, out_blk);
            derive_cascade_key(out_blk, block_bytes, rk);
        } else if (mode == MODE_CBC) {
            for (int i = 0; i < block_bytes; i++) tmp_block[i] = blk[i] ^ prev[i];
            encrypt_block(rk, tmp_block, block_size, out_blk);
            derive_cascade_key(out_blk, block_bytes, rk);
...
static uint8_t *cascade_decrypt(const uint8_t master_key[KEY_SIZE],
    ...
    for (size_t b = 0; b < n_blocks; b++) {
        ...
        if (mode == MODE_ECB) {
            decrypt_block(rk, blk, block_size, out_blk);
...
static uint8_t *cascade_encrypt_ctr_hmac(const uint8_t key[KEY_SIZE],
    ...
    /* cascade key starts from key_crypt */
    uint8_t rk[KEY_SIZE];
...
static uint8_t *cascade_decrypt_ctr_hmac(const uint8_t key[KEY_SIZE],
    ...
    if (file_bytes[MAGIC_LEN] != VERSION_CASC) {
        fprintf(stderr, "ERROR: not a cascade file (version=0x%02x)\n", file_bytes[MAGIC_LEN]);
        return NULL;
    }
...
static double entropy_sample_limit(size_t n) {
    if (n == 0) return 0.0;
    /* Expected number of distinct values: E[D] = 256*(1-(255/256)^n) */
    double distinct = 256.0 * (1.0 - pow(255.0 / 256.0, (double)n));
    if (distinct < 1.0) distinct = 1.0;
    /* Upper bound: log2(distinct) */
...
static void statistics_tests(const uint8_t *plain, size_t plain_len,
    ...
    printf("\n  === STATISTICAL TESTS ===\n");
    /* Warn when sample too small for reliable entropy */
...
static double nist_psi2(const uint8_t *bits, size_t n_bits, int m) {
    if (m <= 0) return 0.0;
    size_t cnt = (size_t)1 << m;
    size_t *freq = (size_t *)calloc(cnt, sizeof(size_t));
    if (!freq) return -1.0;

    for (size_t i = 0; i < n_bits; i++) {
        unsigned pat = 0;
        for (int j = 0; j < m; j++)
            pat = (pat << 1) | (unsigned)nist_get_bit(bits, (i + (size_t)j) % n_bits);
        freq[pat]++;
    }

    double sum = 0.0;
    for (size_t k = 0; k < cnt; k++) {
        double v = (double)freq[k];
        sum += v * v;
    }
    free(freq);
    return (sum * (double)cnt / (double)n_bits) - (double)n_bits;
}
...
static void nist_sts_quick_tests(const uint8_t *cipher, size_t cipher_len) {
    ...
    if (n < 1024) {
        printf("\n  === NIST STS QUICK ===\n");
        printf("  NOTE: too few bits (%zu). Need >= 1024 bits.\n", n);
        return;
    }
    ...
    double p_monobit = erfc(fabs((double)s) / sqrt(2.0 * (double)n));
    nist_print_result("Frequency (Monobit)", p_monobit);
    ...
    /* 2) Block Frequency (M=128) */
    {
        ...
        if (N > 0) {
            ...
            chi2 *= 4.0 * M;
            nist_print_result("Block Frequency (M=128)", chi2_pvalue_approx(chi2, (double)N));
        }
    }
    ...
    /* 3) Runs */
    {
        ...
        if (fabs(pi - 0.5) >= 2.0 / sqrt((double)n)) {
            nist_print_result("Runs", 0.0);
        } else {
            long vn = 1;
            for (size_t i = 1; i < n; i++)
                if (nist_get_bit(cipher, i) != nist_get_bit(cipher, i - 1)) vn++;
    ...
    /* 4) Cumulative sums (forward/backward) */
    {
        long zf = 0, zb = 0, sf = 0, sb = 0;
        for (size_t i = 0; i < n; i++) {
            sf += nist_get_bit(cipher, i) ? 1 : -1;
            long a = labs(sf); if (a > zf) zf = a;
            sb += nist_get_bit(cipher, n - 1 - i) ? 1 : -1;
            a = labs(sb); if (a > zb) zb = a;
        }

        double sum1 = 0.0, sum2 = 0.0;
        if (zf == 0) {
            nist_print_result("Cumulative Sums (forward)", 1.0);
        } else {
            int kmin = (int)(-(double)n / zf + 1.0) / 4;
            int kmax = (int)(((double)n / zf - 1.0) / 4.0);
            for (int k = kmin; k <= kmax; k++) {
                sum1 += normal_cdf((4.0 * k + 1.0) * zf / sqrt((double)n))
                      - normal_cdf((4.0 * k - 1.0) * zf / sqrt((double)n));
                sum2 += normal_cdf((4.0 * k + 3.0) * zf / sqrt((double)n))
                      - normal_cdf((4.0 * k + 1.0) * zf / sqrt((double)n));
            }
        ...
        sum1 = sum2 = 0.0;
        if (zb == 0) {
            nist_print_result("Cumulative Sums (backward)", 1.0);
        } else {
            int kmin = (int)(-(double)n / zb + 1.0) / 4;
            int kmax = (int)(((double)n / zb - 1.0) / 4.0);
            for (int k = kmin; k <= kmax; k++) {
                sum1 += normal_cdf((4.0 * k + 1.0) * zb / sqrt((double)n))
                      - normal_cdf((4.0 * k - 1.0) * zb / sqrt((double)n));
                sum2 += normal_cdf((4.0 * k + 3.0) * zb / sqrt((double)n))
                      - normal_cdf((4.0 * k + 1.0) * zb / sqrt((double)n));
            }
            nist_print_result("Cumulative Sums (backward)", 1.0 - sum1 + sum2);
        }
    }

    /* 5) Approximate Entropy (m=8) */
    {
        if (n < 100000) {
            nist_print_skip("Approximate Entropy (m=8)", "need >=100000 bits");
        } else {
        int m = 8;
        size_t cnt_m = (size_t)1 << m;
        size_t cnt_m1 = (size_t)1 << (m + 1);
        ...
        if (f0 && f1) {
            ...
            for (size_t i = 0; i < cnt_m; i++) {
                if (f0[i]) {
                    double p = (double)f0[i] / (double)n;
                    phi_m += p * log(p);
                }
            }
            for (size_t i = 0; i < cnt_m1; i++) {
                if (f1[i]) {
                    double p = (double)f1[i] / (double)n;
                    phi_m1 += p * log(p);
                }
            }
            double apen = phi_m - phi_m1;
            double chi2 = 2.0 * (double)n * (log(2.0) - apen);
            nist_print_result("Approximate Entropy (m=8)",
                              chi2_pvalue_approx(chi2, (double)((1 << (m - 1)))));
        }
        if (f0) free(f0);
        if (f1) free(f1);
        }
    ...
    {
        if (n < 100000) {
            nist_print_skip("Serial d1 (m=8)", "need >=100000 bits");
            nist_print_skip("Serial d2 (m=8)", "need >=100000 bits");
        } else {
        int m = 8;
        double p2m  = nist_psi2(cipher, n, m);
        double p2m1 = nist_psi2(cipher, n, m - 1);
        double p2m2 = nist_psi2(cipher, n, m - 2);
        if (p2m >= 0.0 && p2m1 >= 0.0 && p2m2 >= 0.0) {
            double d1 = p2m - p2m1;
            double d2 = p2m - 2.0 * p2m1 + p2m2;
            nist_print_result("Serial d1 (m=8)", chi2_pvalue_approx(d1, (double)(1 << (m - 2))));
            nist_print_result("Serial d2 (m=8)", chi2_pvalue_approx(d2, (double)(1 << (m - 3))));
        }
        }
    }
}
...
static void run_self_tests(const uint8_t key[32], EncMode mode) {
    printf("\n%s\n", "────────────────────────────────────────────────────────────");
    printf("  Self-tests  |  mode = %s  [cascade]\n", mode_name(mode));
    printf("%s\n", "────────────────────────────────────────────────────────────");
    ...
    for (int t = 0; t < n; t++) {
        ...
        if (mode == MODE_WBC_CTR_HMAC) {
            ...
            uint8_t *dec = cascade_decrypt(key, enc, enc_len, mode, iv, block_size, &dec_len);
    ...
    for (int si = 0; si < ns; si++) {
        ...
        for (int r = 0; r < repeats; r++) {
            ...
            if (mode == MODE_WBC_CTR_HMAC) {
                ...
                uint8_t *iv = NULL; size_t iv_len = 0;
                enc = cascade_encrypt(key, data, sz, mode, NULL, block_size,
                                   &iv, &iv_len, &enc_len);
                dec = cascade_decrypt(key, enc, enc_len, mode, iv, block_size, &dec_len);
                if (iv) free(iv);
            }
...
/* ── per-block avalanche (full SAC: ALL bits flipped per block) ─────────────
 *
 * FIX: попередня реалізація перевертала лише bit-0 байта-0 кожного блоку,
 * що вимірювало one-bit дифузію (~0.2%), а не повний Strict Avalanche Criterion.
 * Виправлення: для кожного блоку перебираємо ВСІ fixed_bs*8 бітів,
 * шифруємо з кожним перевернутим бітом і усереднюємо результат.
 */
static void per_block_avalanche(const uint8_t key[KEY_SIZE], const uint8_t *data,
    ...
    printf("  Per-block SAC-avalanche (64-byte blocks, all %d bits flipped):\n",
           n_bits);
    uint8_t rk[KEY_SIZE];
    ...
    for (size_t b = 0; b < n_blocks; b++) {
        ...
        encrypt_block(rk, blk, fixed_bs, ct0);

        long long total_diff = 0;
        for (int bit = 0; bit < n_bits; bit++) {
            memcpy(blk1, blk, fixed_bs);
            blk1[bit / 8] ^= (uint8_t)(1u << (bit % 8));
            encrypt_block(rk, blk1, fixed_bs, ct1);
            for (int i = 0; i < fixed_bs; i++)
                total_diff += __builtin_popcount(ct0[i] ^ ct1[i]);
        }
        double pct = (100.0 * (double)total_diff) /
                     ((double)n_bits * (double)n_bits);
        printf("  Block %3zu/%zu:  %5.1f%%\n", b + 1, n_blocks, pct);
        total += pct;
        derive_cascade_key(ct0, fixed_bs, rk);
    }
    printf("  ──────────────────────────────────────────\n");
...
/* ── avalanche test ──────────────────────────────────────────────────────── */
/*
 * FIX (cascade2): для stream-режимів (CTR, CFB, OFB, WBC-CTR-HMAC)
 * шифротекст = відкритий текст XOR keystream.
 * Переворот 1 біта P перевертає рівно 1 біт C — незалежно від алгоритму.
 * Тому plaintext-avalanche для stream-режимів завжди ~0% і безглуздий.
 * ВИПРАВЛЕННЯ: для stream-режимів вимірюємо KEY-avalanche
 * (скільки бітів шифротексту змінюється при зміні 1 біта КЛЮЧА).
 * Для блочних режимів (ECB, CBC) — plaintext-avalanche, як і раніше.
 */
static void avalanche_test(const uint8_t key[KEY_SIZE], const uint8_t *data,
                            size_t data_len, EncMode mode) {
    if (data_len == 0) { printf("  Avalanche effect:    N/A (empty input)\n"); return; }

    /* Визначаємо, чи режим є XOR-потоковим */
    int is_stream = (mode == MODE_CTR  || mode == MODE_CFB ||
                     mode == MODE_OFB  || mode == MODE_WBC_CTR_HMAC);
    ...
    if (mode == MODE_WBC_CTR_HMAC) {
        ...
        nonce0  = enc0 + MAGIC_LEN + 3;
    } else {
        enc0    = cascade_encrypt(key, data, data_len, mode, NULL, block_size,
    ...
    /* Кількість об'єктів для перебору:
     *   stream → KEY_SIZE*8 бітів ключа
     *   block  → data_len*8  бітів відкритого тексту */
    ...
    for (size_t i = 0; i < flip_count; i++) {
        ...
        if (is_stream) {
            /* Перевертаємо i-й біт КЛЮЧА */
            uint8_t mod_key[KEY_SIZE];
            memcpy(mod_key, key, KEY_SIZE);
            mod_key[i / 8] ^= (uint8_t)(1u << (i % 8));

            if (mode == MODE_WBC_CTR_HMAC) {
                enc1 = cascade_encrypt_ctr_hmac(mod_key, data, data_len,
                                                block_size, nonce0, &enc_len1);
                size_t hdr = MAGIC_LEN + 1 + 2 + NONCE_SIZE;
                ...
                enc1 = cascade_encrypt(mod_key, data, data_len, mode, iv0,
                                       block_size, &iv1, &iv1_len, &enc_len1);
                if (enc_len1 == ref_len)
            ...
            /* Перевертаємо i-й біт ВІДКРИТОГО ТЕКСТУ */
            uint8_t *mod = (uint8_t *)malloc(data_len);
            memcpy(mod, data, data_len);
            mod[i / 8] ^= (uint8_t)(1u << (i % 8));

            if (mode == MODE_WBC_CTR_HMAC) {
                enc1 = cascade_encrypt_ctr_hmac(key, mod, data_len,
                                                block_size, nonce0, &enc_len1);
                size_t hdr = MAGIC_LEN + 1 + 2 + NONCE_SIZE;
                const uint8_t *cmp = enc1 + hdr;
                size_t cmp_len = enc_len1 - hdr - MAC_SIZE;
                ...
                uint8_t *iv1 = NULL; size_t iv1_len = 0;
                enc1 = cascade_encrypt(key, mod, data_len, mode, iv0,
                                       block_size, &iv1, &iv1_len, &enc_len1);
                if (enc_len1 == ref_len)
                    for (size_t j = 0; j < enc_len1; j++)
                        total_flips += __builtin_popcount(enc0[j] ^ enc1[j]);
    ...
    double ratio = (double)total_flips /
                   ((double)flip_count * (double)total_bits);
    if (is_stream)
        printf("  Avalanche effect (key):  %.2f%%  "
               "(ideal ~50%%, stream mode — key-bit flip)\n", ratio * 100.0);
    else
...
static void differential_test(const uint8_t key[KEY_SIZE], const uint8_t *data,
    ...
    for (int i = 0; i < KEY_SIZE * 8; i++) {
        ...
        if (mode == MODE_WBC_CTR_HMAC) {
            ...
            uint8_t *iv1 = NULL; size_t iv1_len = 0;
            enc1 = cascade_encrypt(mod_key, data, data_len, mode, iv0, block_size,
...
int main(void) {
    ...
    while (1) {
        ...
        if (choice[0] == '1') {
            ...
            if (mode == MODE_WBC_CTR_HMAC) {
                enc = cascade_encrypt_ctr_hmac(key, (const uint8_t *)text,
                                             text_len, block_size, NULL, &enc_len);
                print_hex("Encrypted HEX:", enc, enc_len);
                dec = cascade_decrypt_ctr_hmac(key, enc, enc_len, &dec_len);
            } else {
                uint8_t *iv = NULL; size_t iv_len = 0;
                enc = cascade_encrypt(key, (const uint8_t *)text, text_len,
                                   mode, NULL, block_size, &iv, &iv_len, &enc_len);
                print_hex("Encrypted HEX:", enc, enc_len);
                dec = cascade_decrypt(key, enc, enc_len, mode, iv, block_size, &dec_len);
                if (iv) free(iv);
            ...
            if (text_len == 0) {
                ...
                /* If input is a pure number, generate that many random bytes */
                char *endp = NULL;
            ...
            if (mode == MODE_WBC_CTR_HMAC) {
                ...
                nist_sts_quick_tests(cipher_payload, cipher_payload_len);
            } else {
