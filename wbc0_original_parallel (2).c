...
static void apply_base_operation_permutation(uint8_t *block, int size, const Operation *op, int op_id, int inverse) {
    ...
    if (strcmp(op->type, "swap") == 0) {
        apply_swap_style(block, size, seed);
        return;
    }
    ...
    a %= size;
    if (a == 0) a = 1;
    int b = (int)((seed >> 8) % (uint32_t)size);

    if (!inverse) {
        rotate_block_bytes(block, size, shift);
        apply_affine_permutation(block, size, a, b, 0);
    } else {
        apply_affine_permutation(block, size, a, b, 1);
        rotate_block_bytes(block, size, -shift);
    }
}
...
static void process_subblock_with_key(WBC1OriginalCipher *cipher,
    ...
    if (!inverse) {
        for (int byte_idx = 0; byte_idx < round_key_len; byte_idx++) {
            ...
            int shift = key_bit ? cipher->cube_d : (cipher->cube_d / 2 + 1);
