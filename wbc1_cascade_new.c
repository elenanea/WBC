...
static uint8_t *cascade_encrypt(const uint8_t master_key[KEY_SIZE],
    ...
    build_op_table(master_key);      /* ключезависимые операции */
    g_perm_bs = -1;                  /* сбрасываем кэш: ключ мог смениться */
    ...
    if (!double_pass) {
        /* Одиночный каскад: inter и есть результат */
        *out_len = padded_len;
        return inter;
    }

    /* ── Проход 2: обратный ECB каскад ──────────────────────────────────── */
...
static uint8_t *cascade_decrypt(const uint8_t master_key[KEY_SIZE],
    ...
    if (double_pass) {
        ...
        /* Одиночный каскад: inter = data */
        memcpy(inter, data, data_len);
    }

    /* ── Отмена прохода 1: прямой каскад ─────────────────────────────────── */
    uint8_t rk[KEY_SIZE];
    memcpy(rk, master_key, KEY_SIZE);
...
static uint8_t *cascade_encrypt_ctr_hmac(const uint8_t key[KEY_SIZE],
                                           ...
                                           size_t *out_len) {
    int block_bytes = block_size;
    build_op_table(key);             /* ключезависимые операции */
    g_perm_bs = -1;                  /* сбрасываем кэш: ключ мог смениться */
    build_perm_table(block_bytes);
