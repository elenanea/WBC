/*
 * CLI MPI wrapper for wbc2_original_parallel.c  (PWBC2 backend)
 * Same protocol as cli_pwbc0.c / cli_pwbc1.c
 * Run:  mpirun -n 4 ./cli_pwbc2 < input > output
 */

#define main wbc2_orig_main_unused
#include "../wbc2_original_parallel.c"
#undef main
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        /* line 3: num_rounds (1-100), default 10 */
        if (fgets(rounds_line, sizeof(rounds_line), stdin) != NULL) {
            rounds_line[strcspn(rounds_line, "\r\n")] = 0;
            int v = atoi(rounds_line);
            if (v >= 1 && v <= 100) num_rounds = v;
        }
    ...
    MPI_Bcast(&num_rounds, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    wbc2_original_init(&cipher, key, 32, block_size_bits, num_rounds);
