/*
 * CLI wrapper for wbc1_cascade_no_mix_FIXED.c (WBC1 - cascade no mix)
 * Usage: cli_wbc1_nomix [enc_mode]
 *   enc_mode (argv[1]): CTR_HMAC (default) | ECB | CBC | CTR | OFB | CFB
 * Protocol (stdin):
 *   Line 1: hex key (64 hex chars)
 *   Line 2: 'e' or 'd'
...
#define main nomix_main_unused
#include "../wbc1_cascade_no_mix_FIXED.c"
#undef main

#include "cli_wbc_serial_common.h"

static void __attribute__((unused)) suppress_unused(void) {
    (void)nomix_main_unused;
}
