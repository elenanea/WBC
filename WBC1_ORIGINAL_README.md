...
data_size_kb:    размер данных в КБ (1-10000 для task 0, 1-1000 для task 1)
...
Оригинальный алгоритм имеет временную сложность **O(n × k)**, где:
- **n** = количество блоков данных
- **k** = количество бит ключа (обычно 256)

Для каждого блока выполняется **256 итераций** (по числу бит ключа), что делает алгоритм медленным для больших объемов данных.

### Рекомендуемые размеры данных

| Режим | Максимум | Причина |
|-------|----------|---------|
| Task 0 (текст) | 10 MB | Быстрая обработка |
| Task 1 (статистика) | 1 MB | Разумное время анализа |

### Оценка времени обработки

| Размер данных | Количество блоков | Операций | Время (прим.) |
|---------------|-------------------|----------|---------------|
| 100 KB | 25,600 | 6.5M | ~1-2 сек |
| 1 MB | 262,144 | 67M | ~1 мин |
| 10 MB | 2,621,440 | 671M | ~10 мин |

**Примечание:** Для больших данных (> 10 MB) рекомендуется использовать улучшенные версии:
- `wbc1_parallel_new.c` - сложность O(n), в 256 раз быстрее
- `wbc1_parallel_cached_opti.c` - оптимизированная версия с кешем
- `wbc1_parallel_gen_cached.c` - параметрическая версия с максимальной стойкостью

### Предупреждения о производительности

При запуске с большими данными программа автоматически показывает предупреждение:

```
⚠ Performance Warning / Предупреждение о производительности:
  Data size: 5000 KB (~1310720 blocks)
  Estimated operations: 0.34 billion (blocks × key_bits)
  This may take several minutes with this algorithm.
  Consider using enhanced versions for large data.
```
