 ...
 *    - Executed via one precomputed composed bit-permutation
 * 
 * Subsequent Rounds (1..N):
 * 1. XOR with round key[i]
 * 2. Apply S-box
...
typedef struct {
    ...
    /* Precomputed composed bit-permutation for round 0 (32 perm+shift steps) */
    uint16_t *round0_bit_fwd; /* maps source bit index -> destination bit index */
    uint16_t *round0_bit_inv; /* inverse mapping */
} WBC2OriginalCipher;
...
static inline int get_block_bit(const uint8_t *block, int bit_index) {
    int byte = bit_index / 8;
    int pos = 7 - (bit_index % 8);
    return (block[byte] >> pos) & 1;
}

static inline void set_block_bit(uint8_t *block, int bit_index, int value) {
    int byte = bit_index / 8;
    int pos = 7 - (bit_index % 8);
    if (value) block[byte] |= (uint8_t)(1U << pos);
    else       block[byte] &= (uint8_t)~(1U << pos);
}
...
/* Diffusion Layer 1: Feistel-based byte-level mixing (easily invertible) */
static void diffusion_layer1(uint8_t *block, int block_size_bytes) {
    /* Feistel structure provides good diffusion and is easily invertible
     * This provides good diffusion at byte level */
    if (block_size_bytes < 4) return;
    
    /* Handle odd-sized blocks by treating last byte separately */
    int working_size = (block_size_bytes % 2 == 0) ? block_size_bytes : block_size_bytes - 1;
    if (working_size < 4) return;
    
    int half = working_size / 2;
    uint8_t *left = block;
    uint8_t *right = block + half;
    uint8_t *temp = (uint8_t *)malloc(half);
    
    /* Feistel round 1: right = right XOR F(left) */
    for (int i = 0; i < half; i++) {
        temp[i] = left[i] ^ left[(i+1) % half] ^ left[(i+2) % half];
    }
    for (int i = 0; i < half; i++) {
        right[i] ^= temp[i];
    }
    
    /* Feistel round 2: left = left XOR F(right) */
    for (int i = 0; i < half; i++) {
        temp[i] = right[i] ^ right[(i+1) % half] ^ right[(i+2) % half];
    }
    for (int i = 0; i < half; i++) {
        left[i] ^= temp[i];
    }
    
    /* Feistel round 3: right = right XOR F(left) for extra diffusion */
    for (int i = 0; i < half; i++) {
        temp[i] = left[i] ^ left[(i+1) % half];
    }
    for (int i = 0; i < half; i++) {
        right[i] ^= temp[i];
    }
...
/* Diffusion Layer 2: Simple byte permutation (self-inverse) */
static void diffusion_layer2(uint8_t *block, int block_size_bytes) {
    /* Use a self-inverse permutation: swap pairs and rotate
     * This is simpler and guaranteed to be invertible by repeating */
    if (block_size_bytes < 2) return;
    
    uint8_t *temp = (uint8_t *)malloc(block_size_bytes);
    memcpy(temp, block, block_size_bytes);
    
    /* Self-inverse permutation: reverse pairs within halves */
    int half = block_size_bytes / 2;
    
    /* First half: reverse pairs */
    ...
    /* Second half: reverse pairs */
    for (int i = half; i < block_size_bytes; i += 2) {
        if (i + 1 < block_size_bytes) {
            block[i] = temp[i + 1];
            block[i + 1] = temp[i];
        } else {
            block[i] = temp[i];
...
/* Correct inverse for diffusion layer 1 using Feistel structure */
static void inverse_diffusion_layer1(uint8_t *block, int block_size_bytes) {
    /* Feistel structure is easily invertible by reversing the rounds */
    if (block_size_bytes < 4) return;
    ...
    /* Reverse Feistel round 3: right = right XOR F(left) */
    for (int i = 0; i < half; i++) {
        temp[i] = left[i] ^ left[(i+1) % half];
    }
    for (int i = 0; i < half; i++) {
    ...
    /* Reverse Feistel round 2: left = left XOR F(right) */
    for (int i = 0; i < half; i++) {
        temp[i] = right[i] ^ right[(i+1) % half] ^ right[(i+2) % half];
    }
    for (int i = 0; i < half; i++) {
    ...
    /* Reverse Feistel round 1: right = right XOR F(left) */
    for (int i = 0; i < half; i++) {
        temp[i] = left[i] ^ left[(i+1) % half] ^ left[(i+2) % half];
    }
    for (int i = 0; i < half; i++) {
...
static void apply_inverse_diffusion(uint8_t *block, int block_size_bytes) {
    /* Reverse layer 2: bitwise diffusion (self-inverse!) */
    diffusion_layer2(block, block_size_bytes);
    
    /* Reverse layer 1: Feistel structure */
    inverse_diffusion_layer1(block, block_size_bytes);
}
...
/* Build composed permutation of round 0 as bit mapping */
static void build_round0_composed_map(WBC2OriginalCipher *cipher) {
    int total_bits = cipher->block_size_bytes * 8;
    cipher->round0_bit_fwd = (uint16_t *)malloc((size_t)total_bits * sizeof(uint16_t));
    cipher->round0_bit_inv = (uint16_t *)malloc((size_t)total_bits * sizeof(uint16_t));

    uint8_t src[MAX_BLOCK_SIZE];

    for (int s = 0; s < total_bits; s++) {
        memset(src, 0, cipher->block_size_bytes);
        set_block_bit(src, s, 1);
        ...
        for (int byte_idx = 0; byte_idx < cipher->key_len_bytes; byte_idx++) {
            ...
            apply_operation(cipher, src, op_id, 0);
            cyclic_bitwise_shift(src, cipher->block_size_bytes, cipher->cube_d);
        }

        int dst = -1;
        for (int d = 0; d < total_bits; d++) {
            if (get_block_bit(src, d)) {
                dst = d;
                break;
            }
        }
        if (dst < 0) dst = s;
        cipher->round0_bit_fwd[s] = (uint16_t)dst;
    }

    for (int i = 0; i < total_bits; i++) cipher->round0_bit_inv[cipher->round0_bit_fwd[i]] = (uint16_t)i;
}

/* Apply composed round-0 mapping in one pass */
static void apply_round0_composed(WBC2OriginalCipher *cipher, uint8_t *block, int inverse) {
    int total_bits = cipher->block_size_bytes * 8;
    const uint16_t *map = inverse ? cipher->round0_bit_inv : cipher->round0_bit_fwd;

    uint8_t out[MAX_BLOCK_SIZE];
    memset(out, 0, cipher->block_size_bytes);

    for (int src = 0; src < total_bits; src++) {
        int bit = get_block_bit(block, src);
        if (bit) set_block_bit(out, map[src], 1);
    }

    memcpy(block, out, cipher->block_size_bytes);
}
...
void wbc2_original_init(WBC2OriginalCipher *cipher, const uint8_t *key, int key_len, int block_size_bits, int num_rounds) {
    ...
    /* Build composed key-dependent map for round 0 (32 perm+shift steps) */
    build_round0_composed_map(cipher);
}
...
void wbc2_original_free(WBC2OriginalCipher *cipher) {
    ...
    if (cipher->round0_bit_fwd) free(cipher->round0_bit_fwd);
    if (cipher->round0_bit_inv) free(cipher->round0_bit_inv);
...
void wbc2_original_encrypt_block(WBC2OriginalCipher *cipher, const uint8_t *plaintext, uint8_t *ciphertext) {
    ...
    /* ========================================================================
     * ROUND 0: ONLY composed key-dependent permutations + shifts
     * ======================================================================== */
    apply_round0_composed(cipher, ciphertext, 0);
    
    /* ========================================================================
     * ROUNDS 1..N: XOR + S-box + Diffusion
     * ======================================================================== */
...
void wbc2_original_decrypt_block(WBC2OriginalCipher *cipher, const uint8_t *ciphertext, uint8_t *plaintext) {
    ...
    /* ========================================================================
     * REVERSE ROUNDS N..1: reverse Diffusion -> inv S-box -> XOR
     * ======================================================================== */
    
    for (int round = cipher->num_rounds; round >= 1; round--) {
        /* Step 3 (reverse): Inverse diffusion */
        apply_inverse_diffusion(plaintext, cipher->block_size_bytes);
        
        /* Step 2 (reverse): Inverse S-box */
        apply_inv_sbox(plaintext, cipher->block_size_bytes, cipher->inv_sbox);
        
        /* Step 1 (reverse): XOR with round key (XOR is self-inverse) */
        xor_round_key(plaintext, cipher->round_keys[round], cipher->block_size_bytes);
    }
    
    /* ========================================================================
     * REVERSE ROUND 0: inverse composed permutations + shifts
     * ======================================================================== */
    apply_round0_composed(cipher, plaintext, 1);
}
...
void parallel_original_encrypt(WBC2OriginalCipher *cipher, const uint8_t *plaintext, int plaintext_len,
    ...
    /* Ensure all ranks use identical length for collective layout */
    int global_plaintext_len = plaintext_len;
    MPI_Bcast(&global_plaintext_len, 1, MPI_INT, 0, MPI_COMM_WORLD);
    
    int block_size = cipher->block_size_bytes;
    int num_blocks = (global_plaintext_len + block_size - 1) / block_size;
    ...
    if (rank == 0) {
        ...
        if (global_plaintext_len > 0) {
            memcpy(padded_plaintext, plaintext, global_plaintext_len);
        }
        /* PKCS7 padding */
        int padding_len = padded_len - global_plaintext_len;
        for (int i = 0; i < padding_len; i++) {
            padded_plaintext[global_plaintext_len + i] = padding_len;
        }
    }
...
void parallel_original_decrypt(WBC2OriginalCipher *cipher, const uint8_t *ciphertext, int ciphertext_len,
    ...
    /* Ensure all ranks use identical length for collective layout */
    int global_ciphertext_len = ciphertext_len;
    MPI_Bcast(&global_ciphertext_len, 1, MPI_INT, 0, MPI_COMM_WORLD);
    
    int block_size = cipher->block_size_bytes;
    int num_blocks = global_ciphertext_len / block_size;
    ...
    if (rank == 0) {
        gathered_plaintext = (uint8_t *)malloc(global_ciphertext_len);
    }
    ...
    if (rank == 0) {
        int padding_len = gathered_plaintext[global_ciphertext_len - 1];
        ...
        if (padding_len > 0 && padding_len <= block_size) {
            ...
            for (int i = global_ciphertext_len - padding_len; i < global_ciphertext_len; i++) {
        ...
        if (valid_padding) {
            *plaintext_len = global_ciphertext_len - padding_len;
        } else {
            *plaintext_len = global_ciphertext_len;  /* No padding to remove */
        }
...
static uint8_t* generate_random_bytes(int size) {
    ...
    /* Fallback path if OpenSSL RNG is unavailable */
    static int seeded = 0;
    if (!seeded) {
...
static void differential_test(WBC2OriginalCipher *cipher, int num_tests, double *results) {
    ...
    for (int test = 0; test < num_tests; test++) {
        ...
        /* Modify one bit in key and fully rebuild dependent structures */
        uint8_t *mod_key = (uint8_t *)malloc(cipher->key_len_bytes);
        memcpy(mod_key, cipher->key, cipher->key_len_bytes);
        int flip_byte = rand() % cipher->key_len_bytes;
        int flip_bit = rand() % 8;
        mod_key[flip_byte] ^= (1 << flip_bit);

        WBC2OriginalCipher mod_cipher;
        wbc2_original_init(&mod_cipher, mod_key, cipher->key_len_bytes,
                          cipher->block_size_bits, cipher->num_rounds);
        
        /* Encrypt with modified key */
        uint8_t *cipher2 = (uint8_t *)malloc(block_size);
        wbc2_original_encrypt_block(&mod_cipher, plaintext, cipher2);
...
static double aggregate_bench_time_local(const double *arr, int n, int trim_outliers) {
    ...
    double s = 0.0;
    int cnt = 0;
    for (int i = lo; i < hi; i++) { s += tmp[i]; cnt++; }
    return (cnt > 0) ? (s / (double)cnt) : tmp[m/2];
}

/* Extended benchmark in WBC1-style format */
static void benchmark_core(WBC2OriginalCipher *cipher) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    for (int si = 0; si < ns; si++) {
        int size = sizes_bytes[si];
        int padded_len = ((size + block_size - 1) / block_size) * block_size;

        uint8_t *plain = NULL;
        uint8_t *plain_padded = NULL;
        int data_ok = 1;
        if (rank == 0) {
            plain = generate_random_bytes(size);
            plain_padded = (uint8_t *)calloc((size_t)padded_len, 1);
            if (!plain || !plain_padded) data_ok = 0;
            if (data_ok) {
                memcpy(plain_padded, plain, (size_t)size);
                int pad = padded_len - size;
                for (int i = 0; i < pad; i++) plain_padded[size + i] = (uint8_t)pad;
            }
        }
        ...
        for (int r = 0; r < repeats; r++) {
            ...
            double t0 = MPI_Wtime();
            if (data_ok && rank == 0)
                parallel_original_encrypt(cipher, plain_padded, size, &enc, &enc_len);
            else
                parallel_original_encrypt(cipher, NULL, 0, &enc, &enc_len);
            MPI_Barrier(MPI_COMM_WORLD);
            if (rank == 0) enc_times[r] = MPI_Wtime() - t0;

            MPI_Barrier(MPI_COMM_WORLD);
            double t1 = MPI_Wtime();
            if (data_ok && rank == 0 && enc)
                parallel_original_decrypt(cipher, enc, enc_len, &dec, &dec_len);
            else
                parallel_original_decrypt(cipher, NULL, 0, &dec, &dec_len);
            MPI_Barrier(MPI_COMM_WORLD);
            if (rank == 0) dec_times[r] = MPI_Wtime() - t1;

            if (rank == 0) {
                if (!dec || !plain || dec_len != size || memcmp(plain, dec, (size_t)size) != 0)
                    all_ok = 0;
            }
        ...
        if (rank == 0) {
            ...
            double enc_kb_s = size_kb / (ea > 0 ? ea : 1e-9);
            double dec_kb_s = size_kb / (da > 0 ? da : 1e-9);
            double enc_mb_s = ((double)size / 1e6) / (ea > 0 ? ea : 1e-9);
            double dec_mb_s = ((double)size / 1e6) / (da > 0 ? da : 1e-9);

            enc_kbs[si] = enc_kb_s;
            dec_kbs[si] = dec_kb_s;

            printf("  %10.2f  %10.5f  %14.2f  %14.2f  %12.4f  %12.4f  %14.4f  %14.4f  %s\n",
                   size_kb, ea, enc_kb_s, dec_kb_s, enc_mb_s, dec_mb_s, enc_mb_s*8.0, dec_mb_s*8.0,
                   all_ok ? "OK" : "FAIL");
        }
...
int main(int argc, char *argv[]) {
    ...
    if (use_named_cli) {
        for (int i = 1; i < argc; i++) {
            if (strcmp(argv[i], "--single") == 0 || strcmp(argv[i], "-s") == 0 || strcmp(argv[i], "--once") == 0) {
                /* Compatibility no-op: WBC2 CLI is already one-shot and has no cascade mode. */
                continue;
            }
            if (strcmp(argv[i], "--task-encrypt") == 0 || strcmp(argv[i], "--encrypt") == 0) {
                task = 0;
                       ...
                       strcmp(argv[i], "--task-stats") == 0) {
                task = 1;
            } else if (strcmp(argv[i], "--task-benchmark") == 0 || strcmp(argv[i], "--benchmark") == 0 || strcmp(argv[i], "--bench") == 0) {
                task = 3;
            } else if (strcmp(argv[i], "--task-table") == 0 || strcmp(argv[i], "--table") == 0 || strcmp(argv[i], "--ops") == 0) {
                task = 2;
                ...
                else if (strcmp(v, "2") == 0 || strcmp(v, "table") == 0 || strcmp(v, "ops") == 0) task = 2;
            } else if ((strcmp(argv[i], "--key-size") == 0 || strcmp(argv[i], "-k") == 0) && i + 1 < argc) {
                key_size = atoi(argv[++i]);
            } else if ((strcmp(argv[i], "--key-source") == 0 || strcmp(argv[i], "-K") == 0) && i + 1 < argc) {
                const char *v = argv[++i];
                if (strcmp(v, "random") == 0 || strcmp(v, "0") == 0) key_source = 0;
                else if (strcmp(v, "file") == 0 || strcmp(v, "1") == 0) key_source = 1;
                else key_source = atoi(v);
            } else if ((strcmp(argv[i], "--block-size") == 0 || strcmp(argv[i], "-b") == 0) && i + 1 < argc) {
                block_size_bits = atoi(argv[++i]);
            } else if ((strcmp(argv[i], "--mode") == 0 || strcmp(argv[i], "-m") == 0) && i + 1 < argc) {
                const char *v = argv[++i];
                if (strcmp(v, "demo") == 0 || strcmp(v, "simple") == 0) mode = 0;
                else if (strcmp(v, "random") == 0 || strcmp(v, "full") == 0) mode = 1;
                else mode = atoi(v);
                ...
                if (rank == 0) {
                    printf("Usage (named): %s --task <encrypt|analysis|benchmark|table> [--key-size 256] [--key-source random] [--block-size 128|0] [--mode demo|random|simple|full] [--size KB] [--rounds N]\n", argv[0]);
                    printf("Aliases: --task-encrypt, --task-analysis, --task-benchmark, --task-table\n");
                    printf("Compatibility: --single and --once are accepted as no-op flags\n");
                    printf("Usage (legacy): %s <task> <key_size> <key_source> <block_size_bits> [mode] [data_size_kb] [num_rounds]\n", argv[0]);
                }
                MPI_Finalize();
                return 0;
            }
    ...
    if (!use_named_cli && argc < 5) {
        if (rank == 0) {
            printf("Usage: %s <task> <key_size> <key_source> <block_size_bits> [mode] [data_size_kb] [num_rounds]\n", argv[0]);
            printf("  task: 0=encrypt/decrypt, 1=statistical tests(+benchmark), 2=print operations table, 3=benchmark only\n");
            ...
            printf("  num_rounds: Optional: number of rounds (default: 6, range: 1-100)\n");
            printf("              presets: 4=fast, 6=balanced (recommended)\n");
            printf("\nExamples:\n");
            printf("  %s 0 256 0 128          # Encrypt demo text (6 rounds default)\n", argv[0]);
            printf("  %s 0 256 0 0 1 10       # Encrypt 10KB with automatic block size\n", argv[0]);
            printf("  %s 0 256 0 128 1 10 4   # Encrypt 10KB random data (4 rounds, fast)\n", argv[0]);
            printf("  %s 0 256 0 128 1 10 6   # Encrypt 10KB random data (6 rounds, balanced)\n", argv[0]);
            ...
            printf("  %s 1 256 0 128 1 100 4  # Statistical tests (4 rounds, fast)\n", argv[0]);
            printf("  %s 1 256 0 128 1 100 6  # Statistical tests (6 rounds, balanced)\n", argv[0]);
    ...
    if (!use_named_cli) {
        ...
        /* Optional: num_rounds (default 6, balanced profile) */
        if (argc >= 8) {
    ...
    if (task == 0) {
        ...
        /* MPI-aware benchmark must be entered by all ranks */
        ...
        /* Benchmark only (MPI-aware, all ranks) */
        benchmark_core(&cipher);
    }
