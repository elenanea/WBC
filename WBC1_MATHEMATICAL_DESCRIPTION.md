...
- **$s$** — размер блока в байтах: $s = 16$ для текущей реализации
- **$n$** — размер блока в битах: $n = 8s = 128$ бит
- **$r$** — номер раунда: $r \in \{1, 2, \ldots, R\}$
- **$R$** — общее количество раундов: $R = 16$ (может быть 16, 32, или 64)
- **$|K|$** — размер ключа в битах: $|K| \in \{128, 192, 256\}$
- **$K$** — мастер-ключ: $K = (k_0, k_1, \ldots, k_{|K|/8-1})$, где $k_i \in \{0,1,\ldots,255\}$
- **$N_{op}$** — количество операций перестановки: $N_{op} = 127$

**Структура куба:** Блок представляется как куб Рубика размером $d_1 \times d_2 \times d_3$, где:
- Для текущей реализации: $d_1 = 2$, $d_2 = 2$, $d_3 = 4$
- Размер блока: $s = d_1 \cdot d_2 \cdot d_3 = 2 \cdot 2 \cdot 4 = 16$ байт

### 1.2. Разбиение текста на блоки

- **$T$** — входной текст: $T = (t_0, t_1, \ldots, t_{|T|-1})$
- **$B_i$** — $i$-й блок текста длиной $s$ байт
- **$n_b$** — количество блоков: $n_b = \lceil |T| / s \rceil$
- **Разбиение:** $T = B_0 \mathbin\Vert B_1 \mathbin\Vert \cdots \mathbin\Vert B_{n_b-1}$
- **Padding:** Если $|T| \not\equiv 0 \pmod{s}$, последний блок дополняется до размера $s$

### 1.3. Состояние блока

- **$B^{(r)}$** — состояние блока после раунда $r$
- **$B^{(0)}$** — открытый текст блока (plaintext): $B^{(0)} = (b_0, b_1, \ldots, b_{s-1})$
- **$B^{(R)}$** — зашифрованный блок (ciphertext)
- **$b_i$** — $i$-й байт блока: $b_i \in \{0,1,\ldots,255\}$, где $i \in \{0, 1, \ldots, s-1\}$

### 1.4. Ключевой материал

- **$RK_r$** — раундовый ключ для раунда $r$: $RK_r = (rk_{r,0}, rk_{r,1}, \ldots, rk_{r,s-1})$
- **$RK_r[j]$** — $j$-й байт раундового ключа $r$: $rk_{r,j} \in \{0,1,\ldots,255\}$, где $j \in \{0, 1, \ldots, s-1\}$
- Генерация: $RK_r = \text{SHA-256}(K \mathbin\Vert r_{BE})$, где $r_{BE}$ — номер раунда в big-endian формате

### 1.5. Табличные структуры
...
- **$\mathcal{P}$** — множество операций перестановок: $\mathcal{P} = \{\pi_0, \pi_1, \ldots, \pi_{N_{op}-1}\}$, где $|\mathcal{P}| = N_{op} = 127$

- **$\pi_i$** — $i$-я операция перестановки байтов: $\pi_i: \{0,\ldots,s-1\} \to \{0,\ldots,s-1\}$
  - **Важно:** $\pi_i$ оперирует **индексами позиций** байтов в блоке, а не их значениями
  - Это биекция (взаимно-однозначное отображение), то есть перестановка $s$ позиций
  - Пример применения к блоку $B = (b_0, b_1, \ldots, b_{s-1})$:
    $$\Pi_i(B) = (b_{\pi_i(0)}, b_{\pi_i(1)}, \ldots, b_{\pi_i(s-1)})$$
  - Конкретный пример для $s=16$: если $\pi_3(0) = 5$, то байт с позиции 5 переместится на позицию 0
  - Основаны на операциях куба Рубика размером $d_1 \times d_2 \times d_3$ ($s$ позиций = $s$ байтов блока)
  
- **$\pi_i^{-1}$** — обратная операция перестановки: $\pi_i^{-1}(\pi_i(j)) = j$ для всех $j \in \{0,\ldots,s-1\}$
  - Восстанавливает исходное расположение байтов: $\Pi_i^{-1}(\Pi_i(B)) = B$
...
### 1.7. Функции алгоритма

- **$\Pi_{op}(B)$** — применение операции перестановки $op \in \{0, 1, \ldots, N_{op}-1\}$ к блоку $B$
- **$\Sigma(B)$** — S-box подстановка: $\Sigma(B) = (S(b_0), S(b_1), \ldots, S(b_{s-1}))$
- **$\Sigma^{-1}(B)$** — обратная S-box подстановка: $\Sigma^{-1}(B) = (S^{-1}(b_0), S^{-1}(b_1), \ldots, S^{-1}(b_{s-1}))$
- **$\Phi(B)$** — диффузия (cumulative XOR)
- **$\Psi_\ell(B)$** — циклический побитовый сдвиг всех байтов блока на $\ell$ бит
...
$$B^{(r)} = (\text{ROT}_{\text{shift}_r}(y^{(r)}_0), \text{ROT}_{\text{shift}_r}(y^{(r)}_1), \ldots, \text{ROT}_{\text{shift}_r}(y^{(r)}_{s-1}))$$

**Полная формула раунда:**
$$B^{(r)} = \Psi_{\text{shift}_r}(\Pi_{op_r}(B^{(r-1)}))$$

**Итоговый зашифрованный блок:**
$$C = B^{(R)}$$
...
$$Y^{(r)} = \Psi_{-\text{shift}_r}(B^{(r)})$$

где $\Psi_{-\ell}$ — сдвиг вправо на $\ell$ бит (обратная операция):
$$\Psi_{-\ell}(B) = (\text{ROT}_{-\ell}(b_0), \text{ROT}_{-\ell}(b_1), \ldots, \text{ROT}_{-\ell}(b_{s-1}))$$

**Шаг 2. Выбор операции:**
$$op_r = RK_r[0] \bmod N_{op}$$

**Шаг 3. Обратная перестановка:**
$$B^{(r-1)} = \Pi_{op_r}^{-1}(Y^{(r)})$$

**Полная формула раунда:**
$$B^{(r-1)} = \Pi_{op_r}^{-1}(\Psi_{-\text{shift}_r}(B^{(r)}))$$
...
$$T_4 = (d^{(2)}_0, d^{(2)}_1, \ldots, d^{(2)}_{s-1})$$

**Шаг 5. Циклический побитовый сдвиг:**
$$\text{shift}_r = (r \bmod 8) + 1$$
$$B^{(r)} = \Psi_{\text{shift}_r}(T_4)$$

**Полная формула раунда MODE 1:**
$$B^{(r)} = \Psi_{\text{shift}_r}(\Phi(\Sigma(\Pi_{op_r}(B^{(r-1)}) \oplus RK_r)))$$
...
$$op_r = RK_r[0] \bmod 127$$
$$B^{(r-1)} = \Pi_{op_r}^{-1}(T_1)$$

**Полная формула раунда MODE 1 (расшифрование):**
$$B^{(r-1)} = \Pi_{op_r}^{-1}((\Phi^{-1}(\Sigma^{-1}(\Psi_{-\text{shift}_r}(B^{(r)}))) \oplus RK_r))$$
...
**Математически:**
$$B^{(r)} = \Psi_{\text{shift}_{r,31}}(\Pi_{op_{r,31}}(\Psi_{\text{shift}_{r,30}}(\Pi_{op_{r,30}}(\cdots \Psi_{\text{shift}_{r,0}}(\Pi_{op_{r,0}}(B^{(r-1)})) \cdots))))$$

Или в компактной форме:
$$B^{(r)} = \left(\prod_{j=31}^{0} \Psi_{\text{shift}_{r,j}} \circ \Pi_{op_{r,j}}\right)(B^{(r-1)})$$
...
**Математически:**
$$B^{(r-1)} = \left(\prod_{j=0}^{31} \Pi_{op_{r,j}}^{-1} \circ \Psi_{-\text{shift}_{r,j}}\right)(B^{(r)})$$
...
$$B^{(r)} = (d^{(2)}_0, d^{(2)}_1, \ldots, d^{(2)}_{s-1})$$

**Полная формула раунда MODE 1:**
$$B^{(r)} = \Phi\left(\Sigma\left(\left[\left(\prod_{j=31}^{0} \Psi_{\text{shift}_{r,j}} \circ \Pi_{op_{r,j}}\right)(B^{(r-1)})\right] \oplus RK_r\right)\right)$$
...
**Полная формула раунда MODE 1 (расшифрование):**
$$B^{(r-1)} = \left(\prod_{j=0}^{31} \Pi_{op_{r,j}}^{-1} \circ \Psi_{-\text{shift}_{r,j}}\right)\left(\Phi^{-1}(\Sigma^{-1}(B^{(r)})) \oplus RK_r\right)$$
...
$$\Psi_{\text{shift}}(X) = (\text{forward}[\text{shift}-1][0][x_0], \text{forward}[\text{shift}-1][1][x_1], \ldots, \text{forward}[\text{shift}-1][15][b_{15}])$$
...
$$B^{(r)} = \left(\prod_{j=31}^{0} \Psi^{\text{cached}}_{\text{shift}_{r,j}} \circ \Pi^{\text{cached}}_{op_{r,j}}\right)(B^{(r-1)})$$

**MODE 1 с кешированием:**
$$B^{(r)} = \Phi\left(\Sigma\left(\left[\left(\prod_{j=31}^{0} \Psi^{\text{cached}}_{\text{shift}_{r,j}} \circ \Pi^{\text{cached}}_{op_{r,j}}\right)(B^{(r-1)})\right] \oplus RK_r\right)\right)$$
...
**Применение к блоку:**
$$\Psi_\ell(X) = (\text{ROT}_r(x_0, \ell), \text{ROT}_r(x_1, \ell), \ldots, \text{ROT}_r(b_{15}, \ell))$$

**Обратная операция:**
$$\Psi_{-\ell}(X) = \Psi_\ell^{-1}(X) = (\text{ROT}_\ell(x_0, \ell), \text{ROT}_\ell(x_1, \ell), \ldots, \text{ROT}_\ell(b_{15}, \ell))$$
...
## 8. ПАРАМЕТРИЧЕСКАЯ ГЕНЕРАТИВНАЯ ВЕРСИЯ

### 8.1. Концепция

В параметрической версии (`wbc1_parallel_gen_cached.c`) операции перестановок не фиксированы, а генерируются динамически в зависимости от ключа, раунда и позиции.

**Ключевая формула:**

$$\pi_i = \pi_i(\theta_{i,r}), \quad \text{где} \quad \theta_{i,r} = \text{PRF}(K, r, i)$$

Где:
- $\pi_i$ — операция перестановки в позиции $i$ раунда $r$
- $\theta_{i,r}$ — параметр операции, зависящий от ключа $K$, раунда $r$ и индекса $i$
- $\text{PRF}$ — псевдослучайная функция (Pseudo-Random Function)

### 8.2. Псевдослучайная функция

$$\text{PRF}(K, r, i) = \text{SHA-256}(K \mathbin\Vert r_{BE} \mathbin\Vert i_{BE} \mathbin\Vert \text{"WBC1\_PARAM"}) \bmod 2^{32}$$

Где $r_{BE}$ и $i_{BE}$ — номер раунда и индекс в big-endian формате.

Выход: 32-битное число $\theta \in \{0, 1, \ldots, 2^{32}-1\}$

### 8.3. Извлечение параметров

Из 32-битного значения $\theta$ извлекаются параметры операции:

$$\begin{aligned}
\text{op\_type} &= \lfloor \theta / 2^{28} \rfloor \bmod 8 \\
\text{param}_1 &= \lfloor \theta / 2^{20} \rfloor \bmod 256 \\
\text{param}_2 &= \lfloor \theta / 2^{12} \rfloor \bmod 256 \\
\text{param}_3 &= \lfloor \theta / 2^{4} \rfloor \bmod 256
\end{aligned}$$

### 8.4. Типы параметрических операций

#### 8.4.1. Параметрический поворот грани

**Параметры:**
- Глубина: $d = (\text{param}_1 \bmod 2) + 1 \in \{1, 2\}$ слоя
- Угол: $\alpha = ((\text{param}_2 \bmod 3) + 1) \cdot 90° \in \{90°, 180°, 270°\}$

**Операция (прямая):**
$$\Pi_{\text{face}}(B, d, \alpha) = \text{cyclic\_shift}(B[\text{face\_indices}(d)], \alpha / 90)$$

**Операция (обратная):**
$$\Pi_{\text{face}}^{-1}(B, d, \alpha) = \Pi_{\text{face}}(B, d, 360° - \alpha)$$

#### 8.4.2. Параметрический срез

**Параметры:**
- Ось: $\text{axis} = \text{param}_1 \bmod 3 \in \{0, 1, 2\}$ (X, Y, Z)
- Толщина: $w = (\text{param}_2 \bmod 3) + 1 \in \{1, 2, 3\}$ слоя
- Направление: $\text{dir} = \text{param}_3 \bmod 2 \in \{0, 1\}$

**Операция (самообратная):**
$$\Pi_{\text{slice}}(B, \text{axis}, w, \text{dir}) = \text{reverse}(B[\text{slice\_indices}(\text{axis}, w)])$$

**Свойство:** $\Pi_{\text{slice}}(\Pi_{\text{slice}}(B)) = B$ (инволюция)

#### 8.4.3. Параметрический паттерн

**Параметры:**
- Seed: $\sigma = (\text{param}_1 \ll 16) \oplus (\text{param}_2 \ll 8) \oplus \text{param}_3$

**Операция (самообратная):**
$$\Pi_{\text{pattern}}(B, \sigma) = \text{apply\_swaps}(B, \text{generate\_swaps}(\sigma))$$

Где $\text{generate\_swaps}(\sigma)$ детерминированно генерирует пары индексов для обмена на основе MT19937 с seed $\sigma$.

**Свойство:** $\Pi_{\text{pattern}}(\Pi_{\text{pattern}}(B)) = B$

### 8.5. Алгоритм шифрования (MODE 1)

$$B^{(r)} = \Phi(\Sigma((B^{(r-1)} \oplus RK_r) \circ \prod_{j=0}^{s-1} \Psi_{\text{shift}(j,r)} \circ \Pi_{\theta_{j,r}}))$$

Где:
- $\theta_{j,r} = \text{PRF}(K, r, j)$
- $\Pi_{\theta_{j,r}}$ — параметрическая операция, зависящая от $\theta_{j,r}$
- Остальные операции как в улучшенной версии

### 8.6. Алгоритм расшифрования (MODE 1)

$$B^{(r-1)} = \prod_{j=s-1}^{0} \Pi_{\theta_{j,r}}^{-1} \circ \Psi_{\text{shift}(j,r)}^{-1} \circ (\Phi^{-1}(\Sigma^{-1}(B^{(r)})) \oplus RK_r)$$

**Важно:** $\theta_{j,r}$ вычисляется идентично для шифрования и расшифрования, обеспечивая детерминизм.

### 8.7. Преимущества параметрического подхода

1. **Бесконечное пространство операций:**
   $$|\text{operations}| = 3 \times 2^{32} \times s \times R = 3 \times 2^{32} \times 16 \times 16 \approx 3.3 \times 10^{12}$$

2. **Ключезависимость:** Операции уникальны для каждого ключа

3. **Устойчивость к криптоанализу:** Нет фиксированной таблицы операций для анализа

4. **Детерминизм:** PRF гарантирует воспроизводимость для корректного расшифрования

5. **Производительность:** Генерация операций практически не влияет на скорость (SHA-256 быстрая)

### 8.8. Сравнение версий

| Характеристика | Базовая | Улучшенная | Оптимизированная | **Параметрическая** |
|----------------|---------|------------|------------------|---------------------|
| Операций/раунд | 1 | 32 | 32 | **32** |
| Кол-во операций | 127 | 127 | 127 | **∞** |
| Ключезависимость | Нет | Нет | Нет | **Да** |
| Скорость | ⚡⚡⚡⚡⚡ | ⚡⚡⚡⚡ | ⚡⚡⚡⚡⚡ | **⚡⚡⚡⚡** |
| Криптостойкость | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | **⭐⭐⭐⭐⭐** |
...
4. **Параметрическая генеративная версия** — ключезависимые операции с бесконечным пространством вариантов
...
**Рекомендации для научной публикации:** 
- **Основная версия:** Параметрическая генеративная MODE 1 (максимальная криптостойкость)
- **Альтернатива:** Улучшенная версия MODE 1 (баланс скорости и стойкости)
- **Оптимизации:** Описать кеширование как способ повышения производительности
